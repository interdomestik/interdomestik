...
test('uses the fixed version-aware role-assumption contract', async () => {
  const source = await readFile(
    new URL('../src/migration-execution-bootstrap.ts', import.meta.url),
    'utf8'
  );
  assert.match(
    source,
    /pg_catalog\.current_setting\('server_version_num'\)::integer\s*>=\s*160000/
  );
  assert.match(source, /pg_catalog\.pg_has_role\([^)]*,\s*'SET'\)/);
  assert.match(source, /pg_catalog\.pg_has_role\([^)]*,\s*'MEMBER'\)/);
});
...
test('allows an owner application object and runs both fixed inventory probes', async () => {
  ...
  // prettier-ignore
  const catalogs = ['pg_class', 'pg_type', 'pg_proc', 'pg_operator', 'pg_collation', 'pg_conversion',
    'pg_opclass', 'pg_opfamily', 'pg_ts_config', 'pg_ts_dict', 'pg_ts_parser', 'pg_ts_template'];
