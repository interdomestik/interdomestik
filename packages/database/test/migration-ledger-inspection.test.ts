...
test('inspects every positive live ledger state without changing the database', async () => {
  const cases: readonly Readonly<{
    ...
    { setup: 'table', applied: 95, ledgerState: 'exact_prefix' },
    { setup: 'table', applied: 96, ledgerState: 'all_applied' },
  ];
  for (const item of cases) {
    ...
    assert.deepEqual(inspection, {
      ...
      summary: {
        ...
        pending_migrations: 96 - item.applied,
...
test('rejects live ACL, shape, overflow and non-prefix states', async () => {
  ...
  const cases: readonly [string, () => Promise<void>][] = [
    ...
    ['MIGRATION_LEDGER_ACL_REJECTED', tableChange(`GRANT TRIGGER ON drizzle.__drizzle_migrations TO ${harness.nonowner}`)],
    ...
    ['MIGRATION_LEDGER_SHAPE_REJECTED', tableChange('ALTER SEQUENCE drizzle.__drizzle_migrations_id_seq AS bigint')],
    ['MIGRATION_LEDGER_SHAPE_REJECTED', tableChange('ALTER SEQUENCE drizzle.__drizzle_migrations_id_seq START WITH 2')],
    ['MIGRATION_LEDGER_SHAPE_REJECTED', tableChange('ALTER SEQUENCE drizzle.__drizzle_migrations_id_seq INCREMENT BY 2')],
    ['MIGRATION_LEDGER_SHAPE_REJECTED', tableChange('ALTER SEQUENCE drizzle.__drizzle_migrations_id_seq MINVALUE 0')],
    ['MIGRATION_LEDGER_SHAPE_REJECTED', tableChange('ALTER SEQUENCE drizzle.__drizzle_migrations_id_seq MAXVALUE 2147483646')],
    ['MIGRATION_LEDGER_SHAPE_REJECTED', tableChange('ALTER SEQUENCE drizzle.__drizzle_migrations_id_seq CACHE 2')],
    ['MIGRATION_LEDGER_SHAPE_REJECTED', tableChange('ALTER SEQUENCE drizzle.__drizzle_migrations_id_seq CYCLE')],
    ...
    ['MIGRATION_LEDGER_PREFIX_REJECTED', async () => { await harness.reset('table'); await harness.fill(97); }],
