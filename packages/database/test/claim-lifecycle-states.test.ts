...
const dropStatusMigrationPath = path.join(
  repoRoot,
...
test('0091 claim status drop migration enforces canonical pairs', () => {
  ...
  assert.match(sql, /ADD CONSTRAINT "claim_lifecycle_pair_check" CHECK/u);
  assert.match(sql, /CASE \("status"::text\)/u);
  assert.match(sql, /"case_lifecycle_state" = 'recovery'/u);
  assert.match(
    sql,
    /"recovery_lifecycle_state" IN \('submitted_to_airline', 'negotiation', 'court'\)/u
  );
  assert.match(
    sql,
    /"case_lifecycle_state" = 'rejected' AND "recovery_lifecycle_state" = 'closed'/u
  );
