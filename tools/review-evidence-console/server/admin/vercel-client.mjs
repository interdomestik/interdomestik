...
export function invokeVercel(args, { stdin = '', spawnImpl = spawn, cwd } = {}) {
  return new Promise((resolve, reject) => {
    const child = spawnImpl('vercel', args, {
      cwd, shell: false, stdio: ['pipe', 'pipe', 'pipe'], env: process.env,
    });
    let stderr = '';
...
export async function readRemoteRegistry({ cwd, invoke = invokeVercel } = {}) {
  ...
  try {
    ...
    const line = (await readFile(target, 'utf8')).split('\n')
...
export async function applyRegistry({
  json, expectedFingerprint, readRemote, fingerprint = value => registryFingerprint(JSON.parse(value)),
  compareAndSwap,
}) {
  ...
  if (await compareAndSwap({ expectedFingerprint, intendedFingerprint: intended, json }) !== true) {
    throw new Error('Refused concurrent registry update.');
  }
  const after = fingerprint(await readRemote());
  if (after !== intended) throw new Error('Registry partial failure: post-write verification differs.');
  return Object.freeze({ fingerprint: after });
}

export function assertProjectPin(project) {
  if (project?.projectId !== PINS.projectId || project?.orgId !== PINS.teamId || project?.projectName !== PINS.projectName) {
