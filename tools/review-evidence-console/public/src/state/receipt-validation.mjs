...
const CORRECTION_FIELDS = ['previousReceiptId', 'correctionItemId', 'correctionReason', 'correctionImpact'];
...
function validateIdentity(receipt, schemaVersion) {
  ...
  if (containsContextualMetadata(receipt)) return invalid('Vërtetimi përmban metadata sugjerimesh jo-kanonike.');
  if (receipt.schemaVersion !== schemaVersion) return failure('schema_mismatch', 'Skema e vërtetimit është e papajtueshme.');
  if (!ID.test(receipt.receiptId) || !Number.isInteger(receipt.receiptVersion) || receipt.receiptVersion < 1)
...
function validateBody(receipt) {
  ...
  if (!SEVERITIES.has(severity) || !Array.isArray(categories) || categories.some(value => typeof value !== 'string'))
    return invalid('Përmbledhja e rrezikut e vërtetimit është e pavlefshme.');
  if (Object.values(receipt.decisions).some(decision =>
    !isRecord(decision) || !hasCanonicalDecisionKeys(decision) || !DECISIONS.has(decision.decision) ||
    !DECISION_SEVERITIES.has(decision.severity) || typeof decision.riskCategory !== 'string' ||
  ...
  if (riskKeys.join(',') !== 'categories,severity' || receipt.riskSummary.severity !== expectedRisk.severity ||
...
function validateCorrection(receipt) {
  ...
  if (receipt.receiptVersion > 1) {
    if (!ID.test(receipt.previousReceiptId) || CORRECTION_FIELDS.slice(1).some(
