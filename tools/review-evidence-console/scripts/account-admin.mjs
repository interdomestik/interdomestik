...
import {
  ...
  mutateAccounts,
  registryFingerprint,
} from '../server/admin/account-mutations.mjs';
import { parseAccountRegistry } from '../server/auth/account-registry.mjs';
import {
  applyRegistry,
  assertProjectPin,
  readRemoteRegistry,
} from '../server/admin/vercel-client.mjs';
...
export function parseAdminArgs(args) {
  const [action] = args;
  if (!['add', 'rotate', 'disable', 'invalidate', 'check'].includes(action))
    throw new TypeError('Invalid action.');
  if (args.includes('--password'))
    throw new TypeError('Password must be supplied through protected stdin.');
  const result = {
    action,
  ...
  if (result.apply && !result.expectedFingerprint)
    throw new TypeError('Apply requires --expected-fingerprint.');
  return result;
}
...
export async function runAdmin(args, env = process.env) {
  ...
  if (options.apply && option(args, '--registry-file'))
    throw new Error('Apply reads only the pinned remote registry.');
  if (options.apply) {
  ...
  if (options.action === 'check')
    return { mode: 'check', fingerprint: registryFingerprint(validated) };
  ...
  if (['add', 'rotate'].includes(options.action)) {
    ...
    operation =
      options.action === 'add'
        ? {
            ...options,
            account: { ...JSON.parse(option(args, '--account-json') ?? '{}'), password },
          }
        : { ...options, password };
  }
  const updated = mutateAccounts(validated, operation);
  if (options.apply) {
