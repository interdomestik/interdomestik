...
test('saves, lists, loads, and exports one verified write-once receipt', async () => {
  ...
  assert.deepEqual(store.save(receipt), { ok: true, value: receipt });
...
test('is idempotent for the same canonical body', async () => {
  const { store, receipt } = await setup();
  assert.equal(store.save(receipt).ok, true);
  assert.deepEqual(store.save(structuredClone(receipt)), { ok: true, value: receipt });
});
...
test('rejects the same ID with a different body', async () => {
  ...
  assert.equal(store.save(changed).code, 'hash_mismatch');
});

test('rejects invalid receipt schema and metadata before storage', async () => {
  const { storage, store, receipt } = await setup();
  assert.equal(store.save({ ...receipt, schemaVersion: 2 }).code, 'schema_mismatch');
  assert.equal(store.save({ ...receipt, packetId: undefined }).code, 'invalid_data');
...
test('maps quota and browser storage exceptions to stable errors', async () => {
  ...
  assert.equal(
    createReceiptStore({ storage: quota, verifyReceipt, schemaVersion: 1 }).save(receipt).code,
    'quota'
  );
