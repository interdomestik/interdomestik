...
const key = composeDraftKey({ assignmentId: bundle.assignment.id,
  reviewerFixtureId: bundle.reviewer.id, packetVersion: bundle.packet.version });
const state = status => Object.fromEntries(
  bundle.packet.itemIds.map(id => [id, { requestedChange: { status } }])
);

test('accepts valid v1 migration input and validated v2 contextual sidecars', () => {
  const store = createDraftStore({ storage: makeStorage(), schemaVersion: 1, contextSchema: schema });
...
test('rejects unknown contextual items, fields, statuses, and unsupported versions', () => {
  ...
  for (const candidate of [
    ...
    { ...base, contextualNoteState: { ...base.contextualNoteState, item_a: { unknown: { status: 'unseen' } } } },
    { ...base, contextualNoteState: { ...base.contextualNoteState, item_a: { requestedChange: { status: 'other' } } } },
