...
function request(path, { method = 'GET', body, cookie, suppliedOrigin = origin } = {}) {
...
test('login, session probe, and logout use a secure private session', async () => {
  ...
  assert.deepEqual({ ...loginBody, draftScope: '<opaque>' }, {
    displayName: 'Gazmend Abazi',
    role: 'governance',
    fixtureId: 'reviewer_governance_mk',
    draftScope: '<opaque>',
...
test('auth failures are generic and mutation boundaries fail closed', async () => {
  ...
  assert.equal(
    (
      await handler(
        request('/api/session/login', { method: 'POST', body: JSON.stringify({ x: 'x'.repeat(9000) }) })
      )
    ).status,
    413
  );
