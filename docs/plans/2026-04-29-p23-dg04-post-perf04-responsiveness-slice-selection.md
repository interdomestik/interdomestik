...
| `apps/web/src/app/[locale]/admin/users/_core.entry.tsx`                                                                              | `/admin/users` is an existing canonical admin surface with server-side search, role, and assignment query handling plus role tabs and user assignment operations.                                                                                                           |
| `apps/web/src/components/admin/users-filters.tsx`                                                                                    | The admin users search, role, and assignment filters still call `router.push` for URL-backed transitions without deterministic pending feedback, active-filter inertness, overlapping-navigation blocking, or same-query no-op behavior.                                    |
...
| ---- | -------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1    | Admin users filter responsiveness                                                                  | Promote. `/admin/users` is an active canonical operator surface, and its search, role, and assignment URL transitions still lack deterministic pending feedback, active-filter inertness, overlap blocking, and same-query no-op behavior. |
