...
with the remainder absorbed by existing headroom. The superseded E2E corpus hash is removed and only
the current hash remains. Executable capacity proof, rather than either historical figure, governs the
candidate; no reserve, deleted-byte credit, allocation reorder or guard weakening is used.
