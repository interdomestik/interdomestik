...
journey/relay/notification-test allocation is separately bounded. Final review hardening consumes
exactly +816 test bytes, +37 package-command bytes and +1 capacity-budget self byte in existing named
allocations; the superseded E2E corpus hash is removed and only the current hash remains. Executable
capacity proof, rather than either historical figure, governs the candidate; no reserve, deleted-byte
credit, allocation reorder or guard weakening is used.
...
the isolated task database is removed after verification. The mounted member return observes exactly
`claim_submitted` and `claim_status_changed` at `/member/claims/[id]`, after the private note, and proves
neither notification exposes the private text. Exact equality avoids SQL wildcard ambiguity. Required
`e2e:gate` and `e2e:gate:pr` commands use one worker, so the seeded member's temporary claim cannot
overlap a sibling gate consumer. The fresh IDA member context uses the established authentication-entry
tenant selector in the header/additional-data contract; ordinary recognized front-door routing does
not derive tenant authority from that header, and the authenticated session is authoritative after
login. This proves a new session and private-note exclusion, not host-derived or header-derived
post-auth tenant authority.
...
| `S3-MEMBER-STAFF-SUPPORTED-PREFIX` | current program; IDA-CLM-001/014; IDA-CAS-005/006/007/008/015 | `gate-ks-sq` plus focused contracts | local  | isolated task DB | pending | local PostgreSQL 16 | `not_applicable` | pending  | Exact mounted UI journey passes with a fresh member return, exact member/agent/branch-manager core denial, private-note and notification-content exclusion, and zero exact-row residue. Exact corrected implementation head `a112029bfd615f1c6d55453abb91837d28de15e2`, tree `7a395e614b905be86e5fb273bf2a6a8817748d8b` and sole E2E corpus tree `192c688bdaee5098c68378ffb93cb457b71a9339` passed `pr:verify`: 1,187 CI, 154 release, 51 required RLS, 3,413 web/12 skips, 81.15% repository line coverage, 263 browser/13 skips and 13 smoke/11 skips; same-head `security:guard` passed. The required RLS command proves live PostgreSQL column types and fails closed without that proof; focused status and communications regressions pass, and the mounted S3 proof uses isolated port 3103 and database `interdomestik_ci_7db57_s3_local_r1`. Protected-base status test identity is independently proven as 794 lines/28,150 bytes/SHA-256 `e9e3d69ecc2a5b41c18bfa6282356377e96da5ac082ed9c5b4456264b379dfea`; the candidate remains 785/28,140. Sonnet 5 and Gemini passed. Independent Astra/high found and verified the retry-worker correction at `3782eb877787b5d8c4087cda09980e0c0aee4`. Fresh receipt `20260916T134723-opus` reached provider-reported `claude-opus-5` with an allowed rate-limit event: its stale proof request is superseded by the exact-head gate above; complete caller inventory proves both shared notification types target the claim-owning member; accepted timestamp-type, mandatory SQL, teardown, sole-hash and private-notification findings are corrected here; transaction rollback, protected-base derivation, core-versus-route proof scope, authentication-entry header scope and named capacity accounting are recorded dispositions rather than product defects. A final current-head Opus resend is deferred until the visible Opus quota resets; protected delivery remains pending and IDA-CLM-010 remains open. |
