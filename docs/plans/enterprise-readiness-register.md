...
| Threat-model evidence        | `docs/plans/ent-tm01-threat-model-evidence-contract-2026-06-05.md`; `docs/plans/ent-tm02-initial-claim-uploads-threat-model-2026-06-05.md`; `docs/plans/ent-tm03-authenticated-claim-evidence-uploads-threat-model-2026-06-06.md`; `docs/plans/ent-tm04-document-signed-urls-and-downloads-threat-model-2026-06-06.md`; `docs/plans/ent-tm05-share-packs-threat-model-2026-06-06.md` | Upload, document-access, and share-pack custody surfaces modeled |
...
| Threat model                 | Evidence contract scoped; initial uploads, authenticated uploads, document access, and share packs modeled | Written per-surface model for registration, uploads, documents, share packs, billing, AI review, and webhooks |
...
- Apply the `ENT-TM01` contract to share-pack creation and bearer-token access only.
- Model authenticated creation, unauthenticated bearer-token retrieval, tenant-scoped document id
  validation, JWT signing, pack expiry/revocation, hashed-token audit logging, STRIDE threats,
...
- The threat-model evidence contract is already defined by `ENT-TM01`, and `ENT-TM02` completed
  ...
  and `ENT-TM04` completed document signed URLs and downloads.
- Share packs are the adjacent custody surface because they expose document metadata through
  bearer tokens and server-side pack state while staying bounded to current route/service proof
  files.
- AI run read and review is the next adjacent sensitive surface from the ownership map and should
  be modeled separately before Paddle webhooks, registration, and admin verification.
