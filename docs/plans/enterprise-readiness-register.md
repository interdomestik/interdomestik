...
| Performance gate evidence    | `docs/plans/ent-perf01-performance-regression-gate-evidence-contract-2026-06-06.md`; `docs/plans/ent-perf02-performance-budget-surface-inventory-and-advisory-dry-run-record-2026-06-06.md`; `docs/plans/ent-perf03-initial-upload-performance-advisory-runner-contract-2026-06-06.md`                                                                                                                                                                                                                                                                                                                                                                                                       | Evidence contract, first surface inventory, and non-blocking runner contract recorded; real advisory dry-run still needs target and fixture proof |
...
| Performance regression gate  | Evidence contract scoped; first `/api/uploads` route/storage surface selected; non-blocking runner contract exists; real advisory dry-run blocked on target, fixture, and session gaps                  | Representative route/storage performance budgets that can block releases                                                                       |
...
- Add a repo-owned non-blocking runner for `POST /api/uploads` advisory measurements.
- Require a non-production target, fixture session, fixture tenant/user identity, and safe output
  path under `tmp/performance`.
- Return structured `blocked` output when prerequisites are missing.
- When prerequisites are supplied, emit aggregate-only p50/p95/p99/error/timeout metrics.
...
- `ENT-PERF01` scoped the performance evidence contract, and `ENT-PERF02` selected `/api/uploads`
  as the first protected route/storage surface.
- `ENT-PERF03` makes the first measurement path repo-owned and safe to run without installing a
  blocking release gate.
- The next smallest repo-owned slice is a real dry-run evidence record that uses the runner against
  an isolated target when fixture prerequisites are available, or records the structured `blocked`
  result when they remain unavailable.
