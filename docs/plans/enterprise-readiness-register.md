...
| Threat model                 | Evidence contract scoped; initial uploads, authenticated uploads, document access, share packs, AI review, Paddle webhooks, and assisted registration modeled | Written per-surface model for registration, uploads, documents, share packs, AI review, billing, and webhooks |
...
- Apply the `ENT-TM01` contract to the agent-assisted member registration route and registration
  write core only.
- Model route rate limits, exact agent-role authorization, host/session/fallback tenant agreement,
  request validation, hard-coded member-role persistence, password hashing, transactional
  user/account/member-number/client-binding/subscription writes, durable agent attribution,
  best-effort welcome email, STRIDE threats, residual risks, and verification evidence.
...
- The threat-model evidence contract is already defined by `ENT-TM01`; `ENT-TM02` through
  `ENT-TM07` completed upload custody, document access, share-pack, AI review, and Paddle webhook
  surfaces.
- Assisted registration is the adjacent sensitive user-creation surface because it crosses
  authenticated agent authorization, tenant source agreement, credential creation, member numbering,
  membership activation, agent attribution, and outbound email boundaries.
- Admin verification details are the next smallest valuable sensitive-route model because they are
  listed separately in the route ownership map and expose privileged verification case detail
  access.
