...
| Performance regression gate  | Evidence contract scoped; first `/api/uploads` route/storage surface selected; non-blocking runner contract exists; blocked dry-run evidence and fixture-readiness handoff recorded for missing target, fixture, and session gaps | Representative route/storage performance budgets that can block releases                                                                       |
...
- Convert the `ENT-PERF04` blocked runner proof into a named fixture-readiness handoff for the next
  advisory latency run.
- Record the missing target URL, fixture tenant id, fixture actor id, fixture session
  creation/expiry owner, storage-provider availability, sample settings, output path, and
  pass/fail owner.
- Keep production load, blocking CI gate installation, runtime behavior, route/auth/tenancy/billing
  product UI/schema/RLS/proxy changes, and signed URL, session-cookie, response-body, request-header,
  or file-content evidence out of scope.
...
- `ENT-PERF03` made the first measurement path repo-owned and safe to run without installing a
  blocking release gate.
- `ENT-PERF04` proved the runner's blocked evidence path in the current repo thread, instead of
  simulating successful latency evidence without target or fixture prerequisites.
- `ENT-PERF05` names the exact target, fixture identity, session, storage, sample-setting, and owner
  prerequisites required before a real advisory latency run can be meaningful.
