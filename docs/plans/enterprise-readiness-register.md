...
- `ENT-PERF05` named the exact target, fixture identity, session, storage, sample-setting, and owner
  prerequisites required before a real advisory latency run can be meaningful.
- `ENT-PERF06` proves those prerequisites are still unavailable in this repo thread and records the
  runner's structured blocked output without simulating latency success.
...
environment-blocked upload performance latency proof, and the promoted
`ENT-SCA04 Deploy Digest Provider Verification Evidence Attempt`.
