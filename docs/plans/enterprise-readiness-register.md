...
| Alert routing proof          | D07 inventory, resolve-threshold disposition, drift-free notification exercise, provider-proof attempt, auth/RLS/protected-route alert coverage contract, current surface inventory, and broader category alert-rule design recorded; routed acknowledgement proof and runtime tag contract remain pending | SLO alerts applied, routed, and exercised for auth, RLS, webhook, and protected-route failure modes                                            |
...
- Define the smallest provider-rule plan for auth/session, RLS or tenant-boundary, and canonical
  protected-route failure modes from the `ENT-ALERT06` contract and `ENT-ALERT07` inventory.
- Decide that current issue fingerprints are useful evidence but not a stable enough first alert
  foundation for all required categories.
- Require a low-cardinality telemetry category contract before provider alert catalog mutation.
...
and the promoted `ENT-ALERT09 Auth RLS Protected Route Telemetry Tag Contract`.
