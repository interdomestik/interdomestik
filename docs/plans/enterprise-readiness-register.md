...
| Performance regression gate  | Evidence contract scoped; first `/api/uploads` route/storage surface selected; non-blocking runner contract exists; blocked dry-run evidence recorded for missing target, fixture, and session gaps     | Representative route/storage performance budgets that can block releases                                                                       |
...
- Execute the repo-owned non-blocking runner for `POST /api/uploads` with a safe output path.
- Record the structured `blocked` output because no non-production target, fixture session, fixture
  tenant id, or fixture actor id is available in this repo thread environment.
...
- `ENT-PERF04` proves the runner's blocked evidence path in the current repo thread, instead of
  simulating successful latency evidence without target or fixture prerequisites.
- The next smallest repo-owned slice is a fixture-readiness handoff that names the missing target,
  fixture identity, session, storage, sample-setting, and owner prerequisites required before a real
  advisory latency run can be meaningful.
