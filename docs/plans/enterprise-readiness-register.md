...
| Alert routing proof          | D07 inventory, resolve-threshold disposition, drift-free notification exercise, provider-proof attempt, auth/RLS/protected-route alert coverage contract, and current surface inventory recorded; routed acknowledgement proof and broader category alert-rule design pending | SLO alerts applied, routed, and exercised for auth, RLS, webhook, and protected-route failure modes                                            |
...
- Record a read-only inventory of current Sentry alert rules, unresolved issue surfaces, repo
  telemetry tags, RLS tests, and protected-route release-gate signals for auth/session, RLS or
  tenant-boundary, and canonical protected-route failure modes.
- Distinguish observable provider issues and release-gate proof from configured routed alert rules.
- Record that only the three D07 provider metric alerts currently exist for the project and that
  dedicated auth/RLS/protected-route alert rules remain unproven.
...
and the promoted `ENT-ALERT08 Auth RLS Protected Route Alert Rule Design`.
