...
# OBR-DG14: Post-T403 T-404 AI Context Required Promotion

## Classification

Classified as promotion/design-gate because this record only reconciles current
plan and tracker authority after `T-403` closeout and promotes the next governed
...
| Source                            | Evidence                                                                                                                                                                                                                                            |
| --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| PR `#1136`                        | `T-403` implementation merged at `818598dadd7733ddf167f790bf8945b9170b0f9d` from implementation head `46a6fdfabdb9c6cfbfcac5788097e3198b9ac616`.                                                                                                    |
| PR `#1137`                        | `T-403` closeout merged at `2f156ea730329416c76742db863b348c5bc6343f`.                                                                                                                                                                              |
| Canonical `main`                  | Clean and synced at `2f156ea730329416c76742db863b348c5bc6343f` before this gate branch.                                                                                                                                                             |
| Current main checks               | At `2f156ea730329416c76742db863b348c5bc6343f`, CI, Sonar Main Gate, CodeQL, and Secret Scan are green. CD remains pending as an external deployment context and is waived only as deployment evidence, not as CI, Sonar, CodeQL, or security proof. |
...
| Architecture tracker              | `T-403` is DONE; `T-404` is TODO, depends only on `T-403`, and requires every `domain-ai` entry point to receive `AICallContext` while `client.ts` rejects calls without it. `T-403b`, `T-405`, and `T-406` remain downstream or dependent.         |
...
- No proxy, canonical route, auth/session, tenant-context, schema, migration,
  RLS, billing/provider, entity migration, or product UI redesign.
...
Current evidence disposition: PR `#1136` and PR `#1137` are merged at the
recorded SHAs. Post-closeout `main` at
`2f156ea730329416c76742db863b348c5bc6343f` has CI, Sonar Main Gate, CodeQL, and
Secret Scan green. CD/deployment evidence remains pending/external and is waived
only as deployment evidence; that waiver does not cover CI, Sonar, CodeQL,
security, reviewer feedback, or future implementation proof.

## Exit State

Authority is reconciled after `T-403`; `T-404` is the only promoted next
