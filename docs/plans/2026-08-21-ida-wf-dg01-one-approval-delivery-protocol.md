# IDA-WF-DG01-ONE-APPROVAL-DELIVERY-R1-TYPED-MODULARITY-REPAIR
...
`B0 authority bootstrap -> B1 CD guard -> S1A skill authority -> S1B model/policy -> S1C typed repo guard -> S2 MCP identity -> S3 exact authority -> S4A terminal delivery -> S4B reviewer policy -> closeout`
...
11. `scripts/repo-size-budget.json`, only if unchanged deterministic sync requires it
...
S1C+ leases bind its digest; missing, invalid, or disallowed policy fails before writes.
...
### S1C — typed repository modularity guard

Exact repository writers:

1. `scripts/modularity-guard-policy.mjs`
2. `scripts/lib/modularity-guard.mjs`
3. `scripts/check-modularity-guard.mjs`
4. `scripts/check-modularity-guard.test.mjs`

The guard classifies changed files before enforcement. For new/refactored production executable
code, `<=150` physical lines is preferred; 151–300 is automatically allowed when function
complexity, duplication, security, tests, and coverage do not regress; above 300 requires a split or
an exposed coherence/risk rationale in PR/review, never minification. Focused tests are `<=300`
physical lines and split by behavior above that limit. JSON receipts/envelopes/config have no line cap but require a closed owning schema, canonical
bytes/hash, and `<=128 KiB`; Markdown gates/protocols require `<=1000` lines and `<=128 KiB` without
removing invariants. Workflow YAML has no arbitrary line cap: changed legacy workflows cannot grow
in complexity, new logic moves to tested helpers, and workflow contracts validate jobs, needs,
permissions, and triggers. Generated/lock files are line-cap exempt but remain covered by existing
integrity/provenance checks. Typed fixtures prove each class and reject unknown/unclassified text.

S1C changes no product, test semantics, required check, workflow, package script, or generated/lock
artifact. Its exact merge consumes the lease; success activates only S2, while any classifier,
budget, schema-owner, provenance, or CI-contract mismatch consumes S1C and blocks successors.
