...
| `pnpm --filter @interdomestik/web lint` on 2026-04-29               | Lint passes with `0` errors and `219` warnings. The warnings are concentrated in `188` `@typescript-eslint/no-explicit-any`, `30` `@typescript-eslint/no-unused-vars`, and `1` stale eslint-disable directive.                                            |
