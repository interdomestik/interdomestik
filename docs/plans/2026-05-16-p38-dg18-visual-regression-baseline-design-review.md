...
- Add a dedicated Playwright project named `visual-crm` or an equivalent narrow visual command that
  runs only CRM visual baseline tests tagged `@crm-visual-baseline`.
- Add a package script such as `e2e:visual:crm` in `apps/web/package.json`; add a root script only if
...
CRM21 should create a visual baseline lane with a stable tag:
...
Mobile and tablet visual baselines require a later slice after the desktop lane proves stable.
...
```bash
pnpm --filter @interdomestik/web test:e2e -- --project=visual-crm --grep '@crm-visual-baseline'
```

Baseline creation should be explicit:

```bash
pnpm --filter @interdomestik/web test:e2e -- --project=visual-crm --grep '@crm-visual-baseline' --update-snapshots
```
...
- wait for fonts and page stability through a helper rather than ad hoc sleeps;
...
The implementation may add a helper such as:
...
If a new Playwright project is added, it should be named `visual-crm` and scoped to
...
- No product UI, route-core, auth, tenancy, schema, scheduler, alerting, or backfill behavior changes
  are introduced.
- Repo-size budget is updated only if the measured screenshot delta requires it.
...
- manual snapshot inspection recorded in PR notes, especially for PII masking and volatile values.
...
6. Should CRM21 add CI-parity Docker requirements for baseline generation?
   - Author recommendation: use existing CI-parity tooling when available, but do not expand Docker
     infrastructure in CRM21 unless implementation proves it is necessary.
...
If the implementation uses existing gate projects instead of a new `visual-crm` project, replace the
