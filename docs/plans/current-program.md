...
both reviewers required. The delivered ceiling rise is +21,952, which is +1,412 above the last
...
skipped) from a clean tree, with `HEAD:apps/web/e2e` equal to the registered tree. Earlier heads
passed the same gates and are superseded, not transferred. Opus `20260920T065438-opus` was the first
completed exact-head review; `20260919T200949-opus` returned `blocked` on the provider usage window
and is not counted. Opus and Codex both required the teardown correction, and Codex additionally
