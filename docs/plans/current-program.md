...
`IDA-WF01-ONE-APPROVAL-DELIVERY` is the only active writer program. The exact gate and envelope
were approved against protected `main@7fb7180aafadf91b79ec37f5daeebaa85bc86ff2`. B0 may materialize
only its eight admitted paths and deterministic non-expanding receipt. Before B0 merge,
`runtime_authorized:false` and `activeSlice:null`; no later child lease is active.
...
- New/refactored production files stay at or below 150 physical lines; focused tests may use the
  admitted 200-line exception only with a recorded reason. No minification.
