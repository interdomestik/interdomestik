...
55. Land `GA04` as the canonical privacy and consent negative-test slice so group admins cannot see claim facts, notes, or documents without explicit member consent.
...
`GA03` Add sponsored-seat activation and self-upgrade path is now complete after completed `GA02`, and `GA04` privacy and consent negative tests are now the next committed priority so group admins still cannot see claim facts, notes, or documents without explicit member consent.
