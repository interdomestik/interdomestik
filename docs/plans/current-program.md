...
54. Land `GA03` as the canonical sponsored-seat activation and self-upgrade slice so sponsored members can activate coverage and move from individual to family coverage without reopening portal or tenancy architecture.
...
`GA03` Add sponsored-seat activation and self-upgrade path is now the next committed priority after completed `GA02`, ahead of `GA04`, so sponsored members can activate their sponsored coverage and self-upgrade without widening the group dashboard beyond aggregate-only visibility.
