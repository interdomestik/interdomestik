...
48. Land `GA01` as the canonical group roster import slice so employers and associations can create sponsored member seats by roster or CSV, replacing the current client-only placeholder before broader `P4G` dashboard, activation, or privacy-consent surfaces are committed.
...
`GA01` Add group roster import is now the next committed priority after the completed `P4` tranche, ahead of `GA02`, `GA03`, and `GA04`, so roster import can replace the current client-only placeholder before broader staff auto-assignment or group pilot surfaces are committed.
