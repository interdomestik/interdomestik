...
| First implementation action requiring RED browser/unit contracts before any other mutation | For the preserved pre-A1 checkpoint only, accepted R2 authorizes controlled checkpoint preservation/rebase onto exact A1 main as the first mutation. After reconciliation, the original RED-before-new-production-edit ordering remains binding: run the corrected focused header-owned RED/GREEN and diagnostic-root collector before any further header edit. |
