# IDA-CI-DG01 — PR Unit Selection R1

## Status and authority

- Classification: parent Tier-3 design gate candidate; pre-authority only.
- Repository authority base: protected `main@faae32d2af477c44d2f2fed6ad36151d08b8ea8d`.
...
- parent gate: `docs/plans/2026-08-20-ida-ci-dg01-pr-unit-selection.md`;
- A1 admission: `docs/plans/2026-08-20-ida-ci-pr-unit-shadow-a1-admission-v1.json`;
- A1 runtime receipt:
  `docs/plans/2026-08-20-ida-ci-pr-unit-shadow-a1-runtime-receipt-r1.json`;
...
1. `docs/plans/2026-08-20-ida-ci-pr-unit-shadow-a1-runtime-receipt-r1.json`;
...
9. `scripts/ci/coverage-gate.mjs`
...
`coverage-gate.mjs` is reserved only for a compatibility-preserving extraction if S1 proves it is
needed; otherwise it remains byte-identical. The new completeness behavior belongs to the explicit
`coverage-summary-contract.mjs` CLI consumed only by the CI unit and full-unit nightly entries.
...
Only after byte-exact R1 approval may those bytes be transferred into a fresh post-R1 A1 child
...
- `coverage-gate.mjs` may only receive a compatibility-preserving extraction and must not grow past
  the inclusive `<=150 physical lines` production ceiling.
...
Any attempt to activate A1 without the separate byte-exact R1 approval, promote A2 without its
