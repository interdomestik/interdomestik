...
**Rev 6 note (2026-06-01):** the tracker adds only targeted governance/legal-entity refinements: `recovery_legal_tenant_id`, case-scoped cross-jurisdiction grants, separation-of-duties, and audited break-glass. It does not reopen routing/auth/tenant architecture or add operational role proliferation to the pilot.

**Rev 22 note (2026-06-10) — Phase-4 integration pass (UI/UX premium mandate · CQRS read models · type-level guards · enterprise-readiness DoD):** four additive integrations; no re-sequencing, no critical-path change, all Phase-C-safe. (a) **UI/UX Premium Overhaul mandate made explicit at M1** (extending the Rev 18 weave): the unified member dashboard — accidents, Green Card, and VONESA flight delays in **one interface with no UI silos** — is composed from a `CaseSummary` discriminated union + static case-kind renderer registry over RSC parallel-route slots, styled by the validated Crystal Home V2 "Matte Layered" design system (`docs/plans/2026-01-28-matte-layered-dashboard-design.md`). This is the platform half of the VONESA business plan's **M1 "Dizajn UX"** phase, and it remains gated behind the front-door/session-context Design Gate (T-011/T-108/T-114) per open decision #10. (b) **CQRS read-model boundary** (§2c): `domain-finance` and `domain-reporting` are outbox-only consumers — **cross-domain database joins are forbidden**; they track the VONESA 30% success fee and marketing/funnel performance purely from `domain_events` projections (tasks CQRS-00…04, ADR-21), aligned with the gated DOM program in `current-program.md`. (c) **Type-level guard layer** (ADR-22): branded `AuthorizedTransition` proof (T-002d), phantom-typed `Signed<>`/`Accepted<>` witnesses + closed `CompensationEvidence` union enforcing no-win-no-fee at compile time (FLIGHT-05b), and brand-minted `AICallContext` (T-403b) — each backing the existing CI greps and runtime checks with compile-time unforgeability. (d) **Enterprise-readiness lanes woven into the Definition of Done** (§6): `ent-tm` per-surface threat models, `ent-perf` latency budgets + the OD#17 frontend budget, `ent-sca` supply-chain attestation, and `ent-dlv` data-lifecycle residue verification riding the T-104d/f/h crypto-shredding machinery.
...
Per the operating-model research (`multi-country-operating-models-2026-05-29.md`): Interdomestik is **one brand, with a locally-registered company per country, each following its own legislation** — the Allianz Partners / Wise / Revolut shape. So `legal_tenant_id` is the membership **entity-of-record**: the company the member actually contracts with, which determines applicable law, terms, tax, and consumer rights. It is **resolved server-side from member residence + chosen plan, never from host**, and is **disclosed** to the member. Recovery can later bind to a different representation entity through `recovery_legal_tenant_id`. This is law, not preference — EU choice-of-law splits a cross-border membership across three jurisdictions at once: **membership contract → Rome I (contracting entity / residence); recovery → Rome II (incident location + recovery/representation entity); data rights → residence (GDPR).** The tenant decomposition therefore exists for legal correctness; collapsing it presents wrong terms/jurisdiction/tax to a member. _(Validate per-country with counsel — the Balkan entities are non-EU but EU-resident diaspora pull EU consumer/data law into scope.)_
...
**UI/UX Premium Overhaul (Rev 22 — the VONESA plan's M1 "Dizajn UX" phase; extends the Rev 18 weave; gated on the Design Gate, OD#10):**

12. **Unified-dashboard composition model — no UI silos.** One server-side projection returns every case as a `CaseSummary` discriminated union (`case_kind: accident | green_card | flight_delay | …`) carrying presentation-safe fields only (lifecycle states, next-step token, document counts — never a price on proof, per M4). Each vertical contributes small RSC leaves registered in a compile-time-exhaustive `Record<CaseKind, CaseKindDescriptor>`; adding a case kind without a descriptor fails the build (the UI twin of `canTransition` exhaustiveness). Verticals never import each other; the design-system layer imports zero domain code. _(T-116.)_
13. **Dashboard shell architecture.** The member dashboard is an RSC shell with parallel-route slots (`@cases` / `@actions` / `@timeline`), each behind its own Suspense boundary; the static chrome prerenders (PPR) and member data streams in. `'use client'` exists only at leaf surfaces. Session + `resolveTenantContext()` are deduped per request via `React.cache()`; the dashboard issues at most two projection queries (case summaries + the T-206 single-query timeline) — no per-card fetch fan-out. The protective disclaimer renders at slot-layout level so no vertical can omit it. _(T-117.)_
14. **Crystal Home V2 design system (`ui/crystal`).** Matte-anchor cards over refractive glass panels per the validated design spec: pure presentational primitives (`MatteAnchorCard`, `RefractiveGlassPanel`, `Stepper`, `Timeline`), each ≤150 lines (T-007b), CSS-only glass with bounded concurrent `backdrop-filter` layers, motion via `LazyMotion` at leaves only, `prefers-reduced-motion` respected, WCAG 2.2 AA contrast, inside the OD#17 budget. The premium look is applied shell-first at M1; domain card surfaces land with their verticals at M2–M4. _(T-118.)_

**Exit criterion:** `ida.interdomestik.com` resolves with **no implied tenant** and is recognized as the canonical neutral host; canonical dashboard/auth E2E setup can run through `ida.localhost` with explicit session/tenant context instead of host identity; country hosts still work but are flagged aliases and no new per-country logic exists; new cases carry incident-country and both lifecycle states; **new memberships capture `legal_tenant_id` + the governing-law/terms snapshot they accepted**; `domain_events` receives a row for every status change; `claim_number`/`tenant.code` uniqueness is enforced; **the unified-dashboard composition model (CaseSummary union + renderer registry + `ui/crystal` primitives) exists with the accident kind registered, builds inside the OD#17 budget, and assumes the neutral front door — no country-host identity anywhere in dashboard IA**. Old rows still readable; the live login has **not** changed.
...
5. **Derive `recovery_law` from `incident_country_code` (Rome II)** and introduce `recovery_legal_tenant_id` — make both explicit case/recovery-level values, distinct from the membership's `legal_tenant_id`, `governing_law`, and from `access_tenant_id`. Define the **out-of-network fallback (critique §5):** when an incident occurs in a country where Interdomestik has no entity, footprint, or partner, recovery cannot silently default to the home entity's law or entity — route to a defined fallback (e.g. guidance-only + documented `no_network` decline reason). Surface a per-jurisdiction note that recovery/success-fee handling may be a regulated legal-services activity that differs by country (flag for review, do not hard-code a single regime). _(See operating-model §5.)_
6. **Home-entity → incident-location handoff as a first-class flow (FIA auto-club pattern).** Membership stays with the home `legal_tenant_id`; when a case is triaged to recovery in a different `incident_country`, the handoff to a local entity/partner is an explicit, audited transition (`recovery.handed_off_to_jurisdiction` event), records `recovery_legal_tenant_id`, and creates a case-scoped access grant. The grant is not an implicit tenant switch and never exposes the full member profile.

**Exit criterion:** Every recovery transition is rejected unless its invariant holds, on **all** write paths; recovery resolves its applicable-law context from `incident_country`, distinct from the membership's `governing_law`; the home→incident handoff emits an audited event, records the recovery entity, creates only case-scoped access, and never mutates the membership's entity-of-record; the success-fee collection is triggered by an event; transition tests cover every edge in §9 of the audit.
...
2. **Key off `access_tenant_id` for isolation, not a generic `tenantId`.** Re-point Postgres isolation / RLS / `withTenant` / `scopeFilter` / document access to `access_tenant_id`; `legal_tenant_id` (membership billing/law) and `recovery_legal_tenant_id` (recovery/representation entity) are resolved separately and may differ. Cross-jurisdiction handoff uses case-scoped grants for the local recovery/legal actor; `global_support` cross-tenant read does not mutate legal entity. A test asserts isolation uses `access_tenant_id` and never a legal/billing/recovery-entity value.
...
5. Document governance boundaries in ADR-09: technical `super_admin` is distinct from global governance and per-country legal-entity administration; separation-of-duties blocks self-approval for fees/payments/terms; break-glass requires reason, expiry, review, and a `security.break_glass_used` audit event. Detailed operational RACI/pricing roles remain a business decision until named owners/processes exist.

**Exit criterion:** `resolveTenantContext()` returns all four contexts; isolation/RLS keys off `access_tenant_id` (test-proven, never legal/billing/recovery-entity values); a stale tenant cookie cannot override the session; a former agent logging into `/member` exercises member scope only; attribution flows to reports without conferring read scope; ADR-09 covers SoD and break-glass without adding unowned pilot roles. Test: "host vs session tenant disagreement" passes.
...
5. **Bind tax/invoicing to the contracting entity — via stored snapshots, not a hardcoded map (feedback #3).** Today `BILLING_ENTITY_BY_TENANT` hardcodes `tenant_ks→ks` etc. and subscription actions derive Paddle config from `ensureTenantId(session)` (`cancel.ts`). Instead, snapshot `legal_tenant_id` + `billing_entity` + `terms_version` onto the `subscriptions` row at creation, derive membership invoicing/tax/currency from that snapshot, and **reconcile Paddle webhooks against the stored snapshot** (not the current session/host). Recovery success-fee invoicing derives from the professional recovery agreement's `recovery_legal_tenant_id`, so a KS membership with MK recovery bills the recovery fee through the MK recovery entity. One member → one membership contracting entity; one recovery matter → one recovery entity; both remain stable even if session/host changes.

**Exit criterion:** A `grace_period` member sees their card; there is no code path that attaches a price to the card; a `domain-ai` call without an `AICallContext` is a type error and a runtime rejection; **every signup, membership view, and invoice shows the contracting entity + governing law**; membership invoices bind to subscription `legal_tenant_id`, recovery success-fee invoices bind to `recovery_legal_tenant_id`, and neither binds to host.
...
- **M5** — Compensation ledger + payout + **Paddle billing-entity snapshots** (membership `legal_tenant_id`; recovery success-fee `recovery_legal_tenant_id`); **Direct Payment Fallback** (declare → fee invoice → reminders → no clean close while fee open); delete superseded code.
...
## 2c. CQRS read-model boundary — Finance / Reporting (Rev 22; DOM-program overlay)

The future decoupled domains (`domain-finance`, `domain-reporting`; later `domain-sales`/`domain-partners` per the gated DOM/OMG programs in `current-program.md`) attach to the spine through **one seam only: the `domain_events` transactional outbox.** Cross-domain database joins are forbidden — these packages never read `claims`, `recovery`, `subscriptions`, or any core table.

**The boundary is enforced three ways:** (1) the DB import-boundary lint (T-302c, extended by CQRS-01) bans importing core Drizzle schemas or raw `dbAdmin`/`dbRls` from the two packages; (2) their database roles carry zero grants on core tables, proven by RLS tests; (3) their packages depend only on the event-relay API plus their own projection tables.

**Consumption mechanics (already live at M1):** each domain is a named consumer in `domain_event_deliveries` with an independent cursor (`FOR UPDATE SKIP LOCKED` selection, idempotent delivery keys reusing the `commercial_action_idempotency` pattern). Delivery is at-least-once; projections are idempotent upserts keyed by `(entity_id, aggregate_version)`. Consumers pin `event_name@event_version`; unknown versions park for review, never guess. Replay from offset zero is the rebuild-and-reconcile mechanism.

**Finance (`fee_receivables` — the VONESA 30% success fee):** projects `recovery.compensation_received` → receivable (fee from the pure passenger-rights ledger math: 30% over the 250/400/600 EC261 tiers), `recovery.fee_calculated` → fee/legal-fee/bank-cost split, `flight.direct_payment_declared` → direct-payment-mode receivable + dunning schedule, `billing.fee_invoice_settled|disputed|written_off` → closure. Finance is **not a second ledger**: any money movement it triggers is a gated command to `domain-billing`, bound to the `legal_tenant_id`/`recovery_legal_tenant_id` snapshots. Reconciliation = replay-from-zero diff plus comparison against Paddle's webhook-reconciled state; a mismatch is an alert, not silent drift.

**Reporting (marketing/funnel):** consumes `assistance.eligibility_checked` (`rule_version`, outcome class), `case.opened`, `flight.*` lifecycle, and the envelope's `booking_tenant_id`/`host_id` attribution facts — attribution flows to reports without ever conferring access (ADR-05). Funnel metrics (Kontrollo→Apliko conversion, cases/month vs the 81/month break-even, cost-per-won-case vs the launch budget) are materialized views over its own projection tables. Zero PII by construction: inputs were allowlisted/redacted at `appendEvent` time, so Reporting cannot leak what it never receives — and GDPR crypto-shredding (T-104d/f) never needs to reach into reporting aggregates.

**Sequencing:** consumer skeletons may register with the M1 outbox plumbing (CQRS-00); projection builds (CQRS-02/03) stay gated behind DOM promotion post-M3/M5. Neither gates the Design Gate, the `ida.*` Go-Live, nor any spine milestone.
...
| ADR-09 | Role separation: `admin` vs `tenant_admin` vs technical `super_admin`, **plus `global_support`/`auditor`**, global governance vs legal-entity administration, SoD limits, and audited break-glass                                                                  | M0→M3   |
...
| ADR-13 | Passenger Rights / VONESA: assignment-default + POA-fallback; `NO ASSIGNMENT/POA = NO AIRLINE SUBMISSION`; `recovery_legal_tenant_id`; `FlightDataProviderPort` + manual-review fallback                                                                           | M2      |
...
| ADR-15 | Compensation ledger + Direct Payment Fallback; membership fees bound to subscription `legal_tenant_id`, recovery success-fees bound to `recovery_legal_tenant_id`, bridged to billing by event                                                                     | M5      |
| ADR-20 | Dynamic theme/branding token resolution from session context (CSS custom properties via Server Components + CDN-served tenant assets; never host; neutral on public; no cross-`access_tenant_id` brand leak). _(ADR-16…19 reserved for the post-M3 OMG program.)_  | M3      |
| ADR-21 | CQRS read-model boundary: `domain-finance`/`domain-reporting` consume `domain_events` only — no cross-domain joins, no core-table grants, no second ledger; projections idempotent + rebuildable; money movement stays a gated `domain-billing` command (§2c)      | M5      |
| ADR-22 | Type-level guard layer: branded `AuthorizedTransition` proof (sole-writer unforgeable), phantom `Signed<>`/`Accepted<>` witnesses + closed `CompensationEvidence` union (no-win-no-fee at compile time), brand-minted `AICallContext`; compile-fail fixtures in CI | M2→M4   |

The repo has an `engineering:architecture` ADR skill available; use it so ADR format stays consistent.

**UI/UX & dashboard modernization (Rev 18) is woven into the milestones, not a separate workstream:** M1 adds the neutral `ida.*` front-door shell + `ida.localhost` as the canonical UI E2E lane (T-115, extends T-108/T-114); M2 streams the events-backed member timeline (T-206); M3 resolves theme/branding tokens from session context (T-310, ADR-20); M4 adds the lint-bounded `useOptimistic` hook (reversible actions only; status-spine mutations stay pessimistic — T-410) and the shared Smart Next Step component library backing SVC-DASH + FLIGHT-09 (T-411); M5 actively deletes `legacy/` and de-duplicates client layouts to 4 shells (T-502). Exit gates: existing `T-007b` 150-line modularity, a size-limit CI budget (Lighthouse mobile > 90, entry bundle < 120 KB gzipped, public-page TTFB < 100ms — Open Decision #17), and consolidated-layout E2E on `ida.localhost`. None extends the critical path. Full node table in the tracker's "UI/UX nodes" section. **Rev 22 extends this weave with the unified-dashboard composition model:** T-116 (`CaseSummary` union + case-kind renderer registry), T-117 (RSC shell + parallel-route slots + PPR + `React.cache()` dedupe), T-118 (`ui/crystal` Crystal Home V2 primitives), T-210 (timeline event-renderer registry over `domain_events`), T-309 (tenant-keyed cache guard: member/tenant-scoped data never enters a shared cache without `access_tenant_id`+`member_id` in the key — the cache twin of the RLS leak harness), and FLIGHT-09b (the VONESA 5-step Kontrollo→Apliko→Kërkesa→Ndjekja→Pagesa stepper as a pure lifecycle projection).
...
state-machine transition tests (case, recovery, membership, documents, attribution); `negotiation`-requires-authorized on every write path; tenant-leak harness across every list query; host-vs-session disagreement resolves to session; `ida.localhost` dashboard/auth lanes establish tenant context explicitly rather than through `ks/mk` hostnames; role-exhaustiveness + role-distinctness; `/member/membership/card` serves active **and** grace_period; banned-framing lint; `compensation guaranteed` absent in every locale; AI-posture-required type+runtime test; audit-event coverage snapshot for every state-changing function; **compile-fail fixtures for the type-level guard layer** (raw status write without an `AuthorizedTransition`, fee invoice without `CompensationEvidence`, airline submission without `Signed<Assignment|PoA>` + `Accepted<FeeAgreement|PrivacyConsent>`, `domain-ai` call without a minted `AICallContext`).

**Enterprise-readiness evidence (Rev 22) — the program is additionally not done until the `ent-*` lanes hold for every surface this program creates or changes:**

- **`ent-tm` (threat models):** every new sensitive surface introduced here has a per-surface threat-model record in the ENT-TM register before its go-live flag flips — at minimum: `domain_events` relay + consumer read paths, `event_pii_references`/key-cache decryption path, SMS-OTP signing (assignment/POA/medical consent), airline submission, compensation-ledger payout + direct-payment declaration, and the Finance/Reporting projection read models (extending ENT-TM01's scope contract).
- **`ent-perf` (performance budgets):** the protected-route p95 budgets from the ENT-PERF contracts cover the unified dashboard's projection queries and the timeline query (one session resolution + ≤2 projection queries per dashboard view, test-proven via T-117), and the OD#17 frontend budget (Lighthouse mobile > 90, entry bundle < 120 KB gzipped, public TTFB < 100ms) is a blocking CI lane — giving the ENT-PERF lane the falsifiable frontend gate it currently lacks.
- **`ent-sca` (supply chain):** CI attestation + deploy-digest verification (ENT-SCA01–04) remain green across the program's new packages and the relay worker; no new package ships outside the attested build path.
- **`ent-dlv` (data lifecycle):** the ENT-DLV01/02 residue contract is executed against the crypto-shredding machinery: a fixture-user erasure proves `domain_event_keys.erased_at` is set, PII references decrypt as `erased_or_unavailable`, timelines render redacted, **zero** plaintext residue remains across events, `audit_log.metadata`, claim columns, `claimDocuments`, storage objects, and `member_notes` (T-104h × T-507), and the allowed-residual list (event skeletons, financial aggregates under Art. 17(3) tax/accounting retention on fee invoices) is enumerated with legal basis and owner sign-off.

At that point the scorecard in §0 reads 10 across the board and `ida.interdomestik.com` can ship without a conflation becoming an incident.
