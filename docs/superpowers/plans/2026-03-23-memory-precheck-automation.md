...
**Goal:** Add a passive, diff-aware memory precheck that surfaces relevant lessons before `pr:verify`, with `pr:verify:hosts` inheriting the same behavior through the existing wrapper flow.
...
- [ ] **Step 5: Wire the script into `pr:verify` so `pr:verify:hosts` inherits the same precheck**
