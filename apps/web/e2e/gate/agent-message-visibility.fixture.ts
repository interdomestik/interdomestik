import { agentClients, claimMessages, claims, db, E2E_USERS, user } from '@interdomestik/database';
...
export async function withAgentMessageFixture<T>(
  ...
  const agent = await db.query.user.findFirst({ where: eq(user.email, email) });
  if (!agent?.tenantId) throw new Error(`Missing seeded agent: ${email}`);
