...
test.describe('pre-membership Free Start recovery', () => {
  ...
  test('keeps later edits across secure promotion and deliberate reset in both orders', async ({ browser }, info) => {
    ...
    await withPage(browser, ida, async keeper => {
      ...
      for (const reverse of [false, true]) { const seed = `Secure seed ${reverse}-${Date.now()}.`, pair = await seededPair(keeper.context(), ida, seed), promotion = `Promotion edit ${reverse}.`, resetEdit = `Reset edit ${reverse}.`; let saved = false; try { await holdLock(pair.first); const save = () => pair.firstOrganizer.getByTestId('free-start-save-open').evaluate(button => (button as HTMLButtonElement).click()), edit = () => pair.secondOrganizer.getByLabel('Brief summary').fill(promotion); if (reverse) { await edit(); await expectPending(pair.first, 1); await save(); } else { await save(); await expectPending(pair.first, 1); await edit(); } await expectPending(pair.first, 2); await releaseLock(pair.first); await expect.poll(() => pair.first.evaluate(key => localStorage.getItem(key), KEY)).toContain(promotion); await expect(pair.firstOrganizer.getByTestId('free-start-save-status')).toHaveAttribute('data-state', 'saved'); saved = true; await expectSurvivor(pair.firstOrganizer, pair.secondOrganizer, promotion, false); await holdLock(pair.first); const reset = () => pair.firstOrganizer.getByTestId('free-start-start-another').evaluate(button => (button as HTMLButtonElement).click()), later = () => pair.secondOrganizer.getByLabel('Brief summary').fill(resetEdit); if (reverse) { await later(); await expectPending(pair.first, 1); await reset(); } else { await reset(); await expectPending(pair.first, 1); await later(); } await expectPending(pair.first, 2); await releaseLock(pair.first); await expect.poll(() => pair.first.evaluate(key => localStorage.getItem(key), KEY)).toContain(resetEdit); await expectPending(pair.first, 0); await expectSurvivor(pair.firstOrganizer, pair.secondOrganizer, resetEdit); } finally { try { const deleted = await deleteSavedDraft(pair.first, pair.firstOrganizer, seed); if (saved) expect(deleted).toBe(true); } finally { await closePair(pair); } } }
    });
  });
