...
// prettier-ignore
async function freshLogout(page: Page, info: TestInfo) { const origin = new URL(String(info.project.use.baseURL)).origin, authOrigin = process.env.BETTER_AUTH_URL?.trim() || origin; const response = await page.request.post(`${origin}/api/auth/sign-out`, { headers: { Origin: authOrigin, 'x-tenant-id': 'tenant_ks' } }); if (!response.ok()) throw new Error(`fresh logout failed: ${response.status()} ${response.statusText()}`); await page.context().clearCookies(); }
// prettier-ignore
