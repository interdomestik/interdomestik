...
// prettier-ignore
async function deleteSavedDrafts(page: Page, organizer: Locator, summaries: string[]) { const failures: string[] = []; for (const summary of summaries) try { expect(await deleteSavedDraft(page, organizer, summary)).toBe(true); } catch (error) { failures.push(`${summary}: ${String(error)}`); } expect(failures, 'every created secure draft must be cleaned').toEqual([]); }
// prettier-ignore
