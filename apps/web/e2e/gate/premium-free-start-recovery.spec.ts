...
// prettier-ignore
async function freshLogin(page: Page, info: TestInfo) { const result = await authPost(info, 'sign-in/email', { email: E2E_USERS.KS_MEMBER.email, password: E2E_PASSWORD, additionalData: { tenantId: 'tenant_ks' } }), token = (result.body as { token?: unknown } | null)?.token, raw = result.setCookies.find(value => /session_token=/i.test(value)); if (!result.ok || typeof token !== 'string' || !raw) throw new Error(`fresh login failed: ${result.status} ${result.statusText}`); const cookie = parseAuthCookie(raw, result.origin), context = page.context(); authStates.set(context, { cookie: cookie.api, token }); await context.addCookies([cookie.browser]); }
// prettier-ignore
...
test.describe('pre-membership Free Start recovery', () => {
  ...
  // prettier-ignore
  test('keeps recovery controls usable at 320-430 effective CSS pixels, 200% presentation and forced accessibility media', async ({ browser }, info) => { const ida = resolveIdaTarget(info); await withPage(browser, ida, async page => { await page.setViewportSize({ width: 640, height: 720 }); await page.context().addCookies([{ domain: new URL(String(ida.project.use.baseURL)).hostname, name: 'cookie_consent', path: '/', sameSite: 'Lax', value: 'necessary' }]); const organizer = await openOrganizer(page, ida), next = organizer.getByRole('button', { name: 'Continue to guided intake' }); await organizer.getByTestId('free-start-category-injury').click(); await next.click(); await organizer.getByLabel('Brief summary').fill('Fractured my arm at work.'); await organizer.getByRole('button', { name: 'Back to claim type' }).click(); await organizer.getByTestId('free-start-category-vehicle').click(); await expect(next).toBeVisible(); await next.click(); await expect(organizer.getByLabel('Brief summary')).toHaveValue(''); await organizer.getByLabel('Brief summary').fill('Accessible recovery facts.'); await expect.poll(() => page.evaluate(key => localStorage.getItem(key), KEY)).toContain('Accessible recovery facts.'); await page.reload(); await page.emulateMedia({ forcedColors: 'active', reducedMotion: 'reduce' }); await page.addStyleTag({ content: '*{line-height:1.5!important;letter-spacing:.12em!important;word-spacing:.16em!important}p{margin-bottom:2em!important}html{zoom:2}' }); const offer = page.getByTestId('anonymous-draft-recovery-offer'), resume = offer.getByRole('button', { name: 'Continue with these notes' }), discard = offer.getByRole('button', { name: 'Discard from this device' }); await expect(offer).toHaveAccessibleName('Continue notes from this browser?'); await resume.focus(); await expect(resume).toBeFocused(); expect(await resume.evaluate(element => { const style = getComputedStyle(element); return style.outlineStyle !== 'none' && style.outlineWidth !== '0px'; })).toBe(true); await page.keyboard.press('Tab'); await expect(discard).toBeFocused(); await expect(discard).toBeVisible(); expect(await discard.evaluate(element => { const style = getComputedStyle(element); return style.outlineStyle !== 'none' && style.outlineWidth !== '0px'; })).toBe(true); for (const width of [320, 360, 390, 430]) { await page.setViewportSize({ width: width * 2, height: 720 }); expect(await offer.evaluate(element => { const viewport = innerWidth, elements = [element, ...element.querySelectorAll('button')]; return document.documentElement.scrollWidth <= document.documentElement.clientWidth && elements.every(item => { const bounds = item.getBoundingClientRect(); return bounds.left >= 0 && bounds.right <= viewport; }); })).toBe(true); } await page.keyboard.press('Shift+Tab'); await expect(resume).toBeFocused(); await page.keyboard.press('Enter'); await expect(offer).toHaveCount(0); }); });
  // prettier-ignore
