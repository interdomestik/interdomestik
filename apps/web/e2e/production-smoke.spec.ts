...
async function loginAs(
  ...
  await page.goto(`/${DEFAULT_LOCALE}/login?tenantId=${tenantId}`);
...
test.describe.serial('Production Smoke Test Plan', () => {
  test.describe('Phase A: Authentication & Routing (Member)', () => {
    test('Member (KS) can login, see dashboard, and create a claim', async ({ page }) => {
      // 1. Login
      await loginAs(page, MEMBER_KS, 'tenant_ks');
  ...
  test.describe('Phase B: Administrative Visibility', () => {
    test('Admin (KS) can find the newly created claim', async ({ page }) => {
      // Login Admin
      await loginAs(page, ADMIN_KS, 'tenant_ks');
      ...
      const searchInput = page.getByPlaceholder(/Search|Kërko/i);
  ...
  test.describe('Phase C: Manager & Isolation', () => {
    ...
    test('Admin (MK) CANNOT view KS claims (Isolation)', async ({ page }) => {
      await loginAs(page, ADMIN_MK, 'tenant_mk');
