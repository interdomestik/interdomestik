...
const claimCopy = {
  ...
  submitDisabled: 'Submit claim — requirements not met', submitExplanation: 'Submitting a claim requires active membership and a complete, saved draft with no unsaved changes. Saving the draft does not submit the claim.', submitUnsavedExplanation: 'Save your changes before submitting. Saving changes does not submit the claim.',
...
describe('ClaimDraftIntake', () => {
  ...
  it('keeps required truth keys present across EN, SQ, SR, and MK', () => {
    ...
    // prettier-ignore
    expect([copies[0].heading, ...copies.flatMap(copy => [copy.submitDisabled, copy.submitExplanation, copy.submitUnsavedExplanation])]).toEqual(['Prepare your claim draft', 'Submit claim — requirements not met', 'Submitting a claim requires active membership and a complete, saved draft with no unsaved changes. Saving the draft does not submit the claim.', 'Save your changes before submitting. Saving changes does not submit the claim.', 'Dorëzo kërkesën — kushtet nuk janë plotësuar', 'Për dorëzim duhen anëtarësim aktiv dhe një skicë e plotë e ruajtur, pa ndryshime të paruajtura. Ruajtja e skicës nuk e dorëzon kërkesën.', 'Ruajini ndryshimet para dorëzimit. Ruajtja e ndryshimeve nuk e dorëzon kërkesën.', 'Поднеси барање — условите не се исполнети', 'За поднесување се потребни активно членство и целосен зачуван нацрт без незачувани измени. Зачувувањето на нацртот не го поднесува барањето.', 'Зачувајте ги измените пред поднесување. Зачувувањето на измените не го поднесува барањето.', 'Podnesi zahtev — uslovi nisu ispunjeni', 'Za podnošenje su potrebni aktivno članstvo i potpun sačuvan nacrt bez nesačuvanih izmena. Čuvanje nacrta ne podnosi zahtev.', 'Sačuvajte izmene pre podnošenja. Čuvanje izmena ne podnosi zahtev.']);
    // prettier-ignore
