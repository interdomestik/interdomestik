...
export async function AgentDashboardView({ locale }: AgentDashboardViewProps) {
                ...
                <div key={claim.id} className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <Button asChild variant="ghost" size="sm">
                      <Link href={`/member/claims/${claim.id}`}>
        ...
        <Card className="lg:col-span-5">
                ...
                <div key={member.id} className="flex items-center gap-3">
                  ...
                  {member.unreadCount && member.alertLink ? (
                    ...
                    <Button asChild size="sm" variant="outline">
                      <Link href={`/member/users/${member.id}`}>{tUsers('view_profile')}</Link>
                    </Button>
                  )}
                </div>
              ))}
          ...
          <CardFooter className="justify-end">
            <Button asChild variant="outline" size="sm">
              <Link href="/member/users">{tMembers('view_all')}</Link>
            </Button>
          </CardFooter>
        </Card>
            ...
            <div className="space-y-4">
              <div className="p-4 rounded-lg bg-yellow-50 border border-yellow-200 text-yellow-900 text-sm">
                <p className="font-semibold mb-1">
                  {t('notes.priority', { defaultValue: 'Priority' })}
                </p>
                <p className="opacity-80">
                  {t('notes.priority_text', { defaultValue: 'High priority items' })}
                </p>
              </div>
              <div className="p-4 rounded-lg bg-blue-50 border border-blue-200 text-blue-900 text-sm">
                <p className="font-semibold mb-1">
                  {t('notes.deadline', { defaultValue: 'Deadline' })}
                </p>
                <p className="opacity-80">
                  {t('notes.deadline_text', { defaultValue: 'Upcoming deadlines' })}
                </p>
              </div>
            </div>
          </CardContent>
