...
export function MemberDashboardV2({ data, locale, tenantId }: MemberDashboardV2Props) {
  ...
  const priorityQuickActionIds = new Set(['qa-help-now', 'qa-start-claim']);
  const priorityQuickActions = quickActions.filter(action =>
    priorityQuickActionIds.has(action.testId)
  );
  const secondaryQuickActions = quickActions.filter(
    action => !priorityQuickActionIds.has(action.testId)
  );
        ...
        <div className="space-y-3.5 sm:space-y-4 lg:space-y-6 lg:col-span-7 xl:col-span-8">
          ...
          >
            ...
            <div data-testid="member-header" className="relative space-y-3">
              ...
              {activeClaimSummary ? (
                <div
                  data-testid="member-hero-claim-spotlight-active"
                  className="inline-flex max-w-full flex-col gap-1.5 rounded-xl border border-emerald-300/35 bg-emerald-500/10 px-3 py-2 shadow-[0_14px_32px_-22px_rgba(16,185,129,0.85)]"
                >
                  <p
                ...
                <div
                  data-testid="member-hero-claim-spotlight-empty"
                  className="inline-flex flex-col gap-2 rounded-xl border border-slate-400/35 bg-white/10 px-3 py-2.5 backdrop-blur"
                >
                  <p className="text-xs font-medium text-slate-100">{t('claims.empty')}</p>
                  <a
                    href={`/${locale}/member/claims/new`}
                    className="inline-flex min-h-10 items-center justify-center rounded-lg border border-white/35 bg-white/10 px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-white/20"
                  >
                    {t('claims.start')}
                  </a>
                </div>
            ...
            <div
              className="mt-4 grid gap-2.5 sm:grid-cols-2 lg:gap-3"
              data-testid="member-primary-actions"
            >
              <a
                data-testid="cta-get-help-now"
                href={contacts.telHref}
                className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-emerald-400 px-4 py-2 text-[0.98rem] font-semibold text-slate-900 shadow-[0_10px_22px_-14px_rgba(16,185,129,0.95)] transition hover:bg-emerald-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/80 lg:px-4 lg:py-2.5"
              >
              ...
              <a
                ...
                href={`/${locale}/member/claims/new`}
                className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-white/30 bg-white/10 px-4 py-2 text-[0.98rem] font-medium text-white shadow-[0_8px_18px_-14px_rgba(255,255,255,0.55)] backdrop-blur transition hover:bg-white/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/80 lg:px-4 lg:py-2.5"
              >
          ...
          >
            ...
            <div
              ...
              data-testid="member-quick-actions-priority"
            >
              {priorityQuickActions.map(action => (
                <a
                  key={action.testId}
                  data-testid={action.testId}
                  data-priority="high"
                  href={action.href}
                  className="group inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-emerald-200 bg-gradient-to-r from-emerald-50 to-cyan-50 px-3 py-2 text-center text-[0.97rem] font-semibold leading-5 text-emerald-900 shadow-[0_12px_24px_-22px_rgba(16,185,129,0.9)] transition hover:-translate-y-0.5 hover:border-emerald-300 hover:shadow-sm max-[375px]:text-[0.92rem]"
                >
                  <span className="inline-flex h-6 w-6 items-center justify-center rounded-lg bg-white/80 transition group-hover:bg-white">
                    <action.icon className="h-[17px] w-[17px] shrink-0 text-emerald-700 transition group-hover:text-emerald-900" />
                  </span>
                  <span>{action.label}</span>
                </a>
              ))}
            </div>
            <div className="mt-3 grid grid-cols-2 gap-2.5 sm:grid-cols-4 sm:gap-3 lg:gap-4">
        ...
        <div className="mt-3.5 space-y-3.5 sm:space-y-4 lg:col-span-5 lg:mt-0 lg:space-y-6 xl:col-span-4">
          ...
          >
            ...
            {activeClaim ? (
              ...
              >
                <p
                  data-testid="claims-module-active-status-pill"
                  className="inline-flex items-center rounded-full border border-emerald-300 bg-white px-2.5 py-1 text-xs font-semibold text-emerald-900"
                >
                  {t('claims.status_label')}: {activeStageLabel}
                </p>
                <p data-testid="claims-module-active-next-step" className="text-sm text-slate-800">
                  {t('claims.next_step_label')}:{' '}
              ...
              <div
                ...
                className="mt-3 space-y-2.5 rounded-xl border border-slate-200 bg-slate-50 p-3"
              >
                <div
                  data-testid="claims-module-empty-illustration"
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <a
                    data-testid="cta-learn-claim-process"
                    href={`/${locale}/member/claim-report`}
                    className="inline-flex min-h-11 items-center rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-100"
                  >
                    {t('qa.incident_guide')}
                  </a>
                </div>
