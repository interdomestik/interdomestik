...
import { confirmAdminUpload, generateAdminUploadUrl } from './actions';

describe('admin claim upload actions', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ...
    hoisted.findClaimFirst.mockResolvedValue({ id: 'claim-1', tenantId: 'tenant-1' });
  ...
  it('creates an upload URL for an admin-visible claim', async () => {
  ...
  it('rejects uploads when the host tenant drifts from the session tenant', async () => {
  ...
  it('persists metadata and revalidates admin, member, and staff claim surfaces', async () => {
