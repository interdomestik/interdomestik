...
describe('anonymous recovery race contracts', () => {
  ...
  // prettier-ignore
  it('sanitizes an injury epoch before an eligible recovery write', async () => { const hook = setupRecovery(); await waitFor(() => expect(hook.result.current.ready).toBe(true)); const medical = { ...snapshot.draft, summary: 'Fractured my arm at work.' }; hook.rerender({ ...hook.props, category: 'injury', draft: medical }); await act(async () => Promise.resolve()); hook.rerender({ ...hook.props, category: 'vehicle', draft: medical }); await waitFor(() => expect(hook.onRestore).toHaveBeenCalledWith({ category: 'vehicle', draft: { counterparty: '', desiredOutcome: '', incidentDate: '', issueType: '', summary: '' }, resumeStep: 'details' })); expect(localStorage.getItem(ANONYMOUS_DRAFT_KEY) ?? '').not.toContain('Fractured'); });
  // prettier-ignore
