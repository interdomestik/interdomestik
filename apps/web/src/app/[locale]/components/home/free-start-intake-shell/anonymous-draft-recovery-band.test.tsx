...
describe('anonymous recovery race contracts', () => {
  ...
  // prettier-ignore
  it('gates a held cold read and binds resume to the current form context', async () => { writeAnonymousDraft(localStorage, snapshot, null); let grant!: () => void; const hold = () => vi.fn((_name, _options, callback) => new Promise(resolve => { grant = () => resolve(callback()); })); let request = hold(); Object.defineProperty(navigator, 'locks', { configurable: true, value: { request } }); const hook = setupRecovery(); await waitFor(() => expect(request).toHaveBeenCalledOnce()); expect(hook.result.current.ready).toBe(false); act(() => grant()); await waitFor(() => expect(hook.result.current.state).toBe('offer')); expect(hook.result.current.ready).toBe(true); request = hold(); Object.defineProperty(navigator, 'locks', { configurable: true, value: { request } }); act(() => hook.result.current.resume()); await waitFor(() => expect(request).toHaveBeenCalledOnce()); hook.rerender({ ...hook.props, activeId: 'server', category: 'property', draft: { ...snapshot.draft, summary: 'Server draft.' }, lifecycleState: 'saved' }); act(() => grant()); await act(async () => Promise.resolve()); expect(hook.onRestore).not.toHaveBeenCalled(); expect(hook.result.current.state).toBe('offer'); });
  // prettier-ignore
  it('binds queued secure promotion and discard to their originating draft epoch', async () => { writeAnonymousDraft(localStorage, snapshot, null); const hook = setupRecovery(); await waitFor(() => expect(hook.result.current.state).toBe('offer')); act(() => hook.result.current.resume()); await waitFor(() => expect(hook.onRestore).toHaveBeenCalled()); const restored = { ...hook.props, category: 'property' as const, step: 'preview' as const }; hook.rerender(restored); hook.rerender({ ...restored, activeId: 'server-a', lifecycleState: 'saving' }); let grant!: () => void; const held = vi.fn((_name, _options, callback) => new Promise(resolve => { grant = () => resolve(callback()); })); Object.defineProperty(navigator, 'locks', { configurable: true, value: { request: held } }); hook.rerender({ ...restored, activeId: 'server-a', lifecycleState: 'saved' }); await waitFor(() => expect(held).toHaveBeenCalledOnce()); hook.rerender({ ...restored, activeId: 'server-b', draft: { ...snapshot.draft, summary: 'Secure draft B.' }, lifecycleState: 'saved' }); act(() => grant()); await act(async () => Promise.resolve()); expect(localStorage.getItem(ANONYMOUS_DRAFT_KEY)).not.toContain('Secure draft B.'); hook.unmount(); localStorage.clear(); writeAnonymousDraft(localStorage, snapshot, null); Object.defineProperty(navigator, 'locks', { configurable: true, value: { request: vi.fn(async (_name, _options, callback) => callback()) } }); const discard = setupRecovery(); await waitFor(() => expect(discard.result.current.state).toBe('offer')); let clear!: () => void; const clearHeld = vi.fn((_name, _options, callback) => new Promise(resolve => { clear = () => resolve(callback()); })); Object.defineProperty(navigator, 'locks', { configurable: true, value: { request: clearHeld } }); act(() => discard.result.current.discard()); await waitFor(() => expect(clearHeld).toHaveBeenCalledOnce()); const secureB = { ...discard.props, activeId: 'server-b', category: 'property' as const, draft: { ...snapshot.draft, summary: 'Secure draft B.' }, lifecycleState: 'saved' as const }; discard.rerender(secureB); act(() => clear()); await act(async () => Promise.resolve()); expect(discard.onReset).not.toHaveBeenCalled(); expect(localStorage.getItem(ANONYMOUS_DRAFT_KEY)).toContain(snapshot.draft.summary); act(() => discard.result.current.discard()); await waitFor(() => expect(clearHeld).toHaveBeenCalledTimes(2)); discard.rerender({ ...secureB, lifecycleState: 'loading' }); discard.rerender(secureB); act(() => clear()); await act(async () => Promise.resolve()); expect(discard.onReset).not.toHaveBeenCalled(); expect(localStorage.getItem(ANONYMOUS_DRAFT_KEY)).toContain(snapshot.draft.summary); });
  // prettier-ignore
