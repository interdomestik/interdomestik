...
// prettier-ignore
const RECOVERY_MEDICAL_WORD = /^(?:injur\p{L}*|fractur\p{L}*|concuss\p{L}*|break(?:s|ing)?|broke|broken|doctor(?:s|ed|ing)?|hospital(?:s|i[sz](?:e|ed|es|ing|ations?))?|pain(?:s|ful(?:ly)?)?|blood\p{L}*|bleed\p{L}*|bled|medic(?:s|al\p{L}*|in\p{L}*|at\p{L}*)?|diagnos\p{L}*|treat(?:s|ed|ing|ment|ments)?|wound\p{L}*|ambulanc\p{L}*|surger\p{L}*|lënd(?:im|uar|oh)\p{L}*|mjek\p{L}*|spital\p{L}*|dhimb\p{L}*|gjak\p{L}*|diagnoz\p{L}*|trajt\p{L}*|povre[dđ]\p{L}*|lekar\p{L}*|doktor\p{L}*|bolnic\p{L}*|bol(?:a|i|om|ova|ove|ovi|u)?|krv\p{L}*|medicin\p{L}*|dijagnoz\p{L}*|le[cč]en\p{L}*|prelom\p{L}*|повре[дђ]\p{L}*|лекар\p{L}*|доктор\p{L}*|болниц\p{L}*|бол(?:а|и|ом|ова|ове|ови|у)?|болк\p{L}*|крв\p{L}*|медицин\p{L}*|дијагноз\p{L}*|лекува\p{L}*|леч\p{L}*|прелом\p{L}*)$/iu;
// prettier-ignore
