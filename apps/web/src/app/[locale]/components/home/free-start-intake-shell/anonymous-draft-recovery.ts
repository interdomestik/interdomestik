...
// prettier-ignore
const RECOVERY_MEDICAL_WORD = /^(?:injur\p{L}*|fractur\p{L}*|concuss\p{L}*|doctor(?:s|ed|ing)?|hospital(?:s|i[sz](?:e|ed|es|ing|ations?))?|pain(?:s|ful(?:ly)?)?|blood\p{L}*|bleed\p{L}*|bled|medic(?:s|al\p{L}*|in\p{L}*|at\p{L}*)?|diagnos\p{L}*|treat(?:s|ed|ing|ment|ments)?|wound\p{L}*|ambulanc\p{L}*|surger\p{L}*|cancer\p{L}*|chemotherap\p{L}*|tumou?r\p{L}*|disease\p{L}*|illness\p{L}*|stroke\p{L}*|seizure\p{L}*|lënd(?:im|uar|oh)\p{L}*|mjek\p{L}*|spital\p{L}*|dhimb\p{L}*|gjak\p{L}*|diagnoz\p{L}*|trajt\p{L}*|povre[dđ]\p{L}*|lekar\p{L}*|doktor\p{L}*|bolnic\p{L}*|bol(?:a|i|om|ova|ove|ovi|u)?|krv\p{L}*|medicin\p{L}*|dijagnoz\p{L}*|le[cč]en\p{L}*|prelom\p{L}*|повре[дђ]\p{L}*|лекар\p{L}*|доктор\p{L}*|болниц\p{L}*|бол(?:а|и|ом|ова|ове|ови|у)?|болк\p{L}*|крв\p{L}*|медицин\p{L}*|дијагноз\p{L}*|лекува\p{L}*|леч\p{L}*|прелом\p{L}*)$/iu;
// prettier-ignore
function containsForbiddenFact(...values: Array<string | null | undefined>) { return values.some(value => { const text = value?.normalize('NFKC').toLowerCase() ?? '', words = text.match(/\p{L}+/gu) ?? []; return words.some(word => RECOVERY_MEDICAL_WORD.test(word)) || /(?:\b(?:broke|broken)\s+(?:(?:my|your|his|her|their|the|an?)\s+)?(?:arm|leg|bone|rib|wrist|ankle|hand|foot|finger|toe|nose|jaw|neck|back)\b|[\w.+-]+@[\w.-]+\.[a-z]{2,}|<\/?[a-z][^>]*>|&(?:lt|gt|#x?0*3[ce]);|\b(?:otp|verification\s+code|one[- ]time\s+(?:code|password))\s*[:#-]?\s*\d{4,8}\b|\b(?:tenant|member|user|session|claim|case)[_-][\p{L}\p{N}-]{2,}\b|\b(?:tenant|member|user|session|claim|case)\s+(?:id|number|no\.?)\s*[:#-]?\s*[\p{L}\p{N}-]{3,}\b|\beyj[a-z0-9_-]{10,}\.[a-z0-9_-]{10,}\.[a-z0-9_-]{10,}\b)/iu.test(text); }); }
// prettier-ignore
