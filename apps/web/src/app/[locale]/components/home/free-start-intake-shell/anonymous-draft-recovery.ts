...
// prettier-ignore
const MEDICAL_PREFIXES = ['injur', 'fractur', 'concuss', 'blood', 'bleed', 'medic', 'diagnos', 'wound', 'ambulanc', 'surger', 'cancer', 'chemotherap', 'tumor', 'tumour', 'disease', 'illness', 'stroke', 'seizure', 'lëndim', 'lënduar', 'lëndoh', 'mjek', 'spital', 'dhimb', 'gjak', 'diagnoz', 'trajt', 'povred', 'povređ', 'lekar', 'doktor', 'bolnic', 'krv', 'medicin', 'dijagnoz', 'lecen', 'lečen', 'prelom', 'повред', 'повређ', 'лекар', 'доктор', 'болниц', 'болк', 'крв', 'медицин', 'дијагноз', 'лекува', 'леч', 'прелом'], MEDICAL_WORDS = new Set(['bled', 'doctor', 'doctors', 'doctored', 'doctoring', 'hospital', 'hospitals', 'hospitalise', 'hospitalised', 'hospitalises', 'hospitalising', 'hospitalisation', 'hospitalisations', 'hospitalize', 'hospitalized', 'hospitalizes', 'hospitalizing', 'hospitalization', 'hospitalizations', 'pain', 'pains', 'painful', 'painfully', 'treat', 'treats', 'treated', 'treating', 'treatment', 'treatments', 'bol', 'bola', 'boli', 'bolom', 'bolova', 'bolove', 'bolovi', 'bolu', 'бол', 'бола', 'боли', 'болом', 'болова', 'болове', 'болови', 'болу']), BODY_PARTS = new Set(['arm', 'leg', 'bone', 'rib', 'wrist', 'ankle', 'hand', 'foot', 'finger', 'toe', 'nose', 'jaw', 'neck', 'back']), BODY_DETERMINERS = new Set(['my', 'your', 'his', 'her', 'their', 'the', 'a', 'an']), OTP_LABELS = ['otp', 'verification code', 'one-time code', 'one time code', 'one-time password', 'one time password'], SENSITIVE_LABELS = new Set(['tenant', 'member', 'user', 'session', 'claim', 'case']), ID_QUALIFIERS = new Set(['id', 'number', 'no']), SENSITIVE_PATTERNS = [/<\/?[a-z][^>]*>/iu, /&(?:lt|gt|#x?0*3[ce]);/iu, /\b(?:tenant|member|user|session|claim|case)[_-][\p{L}\p{N}-]{2,}\b/iu, /\beyj[a-z0-9_-]{10,}\.[a-z0-9_-]{10,}\.[a-z0-9_-]{10,}\b/iu], isMedicalWord = (word: string) => MEDICAL_WORDS.has(word) || MEDICAL_PREFIXES.some(prefix => word.startsWith(prefix)), containsBrokenBody = (words: string[]) => words.some((word, index) => { if (word !== 'broke' && word !== 'broken') { return false; } const next = BODY_DETERMINERS.has(words[index + 1] ?? '') ? index + 2 : index + 1; return BODY_PARTS.has(words[next] ?? ''); }), containsLabeledId = (tokens: string[]) => tokens.some((token, index) => SENSITIVE_LABELS.has(token) && ID_QUALIFIERS.has(tokens[index + 1] ?? '') && (tokens[index + 2]?.length ?? 0) >= 3);
// prettier-ignore
function containsForbiddenFact(...values: Array<string | null | undefined>) { return values.some(value => { const text = value?.normalize('NFKC').toLowerCase() ?? '', words = text.match(/\p{L}+/gu) ?? [], tokens = text.match(/[\p{L}\p{N}]+/gu) ?? [], hasOtp = OTP_LABELS.some(label => text.includes(label)) && /\d{4,8}/u.test(text); return words.some(isMedicalWord) || containsBrokenBody(words) || containsLabeledId(tokens) || text.includes('@') || hasOtp || SENSITIVE_PATTERNS.some(pattern => pattern.test(text)); }); }
// prettier-ignore
const recoverableDraftSchema = freeStartDraftPayloadSchema.refine(value => !containsForbiddenFact(value.counterparty, value.summary), { message: 'removeSensitiveDetails' });
