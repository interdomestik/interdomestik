...
describe('anonymous Free Start recovery', () => {
  ...
  // prettier-ignore
  it('settles sibling storage propagation before cleaning invalid data', async () => { const props: HookProps = { activeId: null, category: null, draft: snapshot.draft, lifecycleState: 'idle', neutralHost: globalThis.location.host, onReset: vi.fn(), onRestore: vi.fn(), resetCategory: null, step: 'details' }; const hook = renderHook(value => useAnonymousDraftRecovery(value), { initialProps: props }); await waitFor(() => expect(hook.result.current.ready).toBe(true)); vi.useFakeTimers(); localStorage.setItem(ANONYMOUS_DRAFT_KEY, '{'); act(() => globalThis.dispatchEvent(new StorageEvent('storage', { key: ANONYMOUS_DRAFT_KEY }))); await act(async () => Promise.resolve()); expect(localStorage.getItem(ANONYMOUS_DRAFT_KEY)).toBe('{'); await act(async () => vi.runAllTimersAsync()); expect(localStorage.getItem(ANONYMOUS_DRAFT_KEY)).toBeNull(); });
  // prettier-ignore
