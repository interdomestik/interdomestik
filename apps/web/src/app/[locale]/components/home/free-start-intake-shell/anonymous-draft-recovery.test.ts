...
// prettier-ignore
function storageEvent(key: string | null, storageArea?: Storage, newValue: string | null = 'changed') { const event = new Event('storage') as StorageEvent; Object.defineProperties(event, { key: { value: key }, newValue: { value: newValue }, storageArea: { value: storageArea } }); return event; }
// prettier-ignore
...
// prettier-ignore
const invalidRecords = [['malformed', '{'], ['unknown version', rawRecord({ version: 2 })], ['expired', rawRecord({ expiresAt: new Date(NOW - 1).toISOString() })], ['extended TTL', rawRecord({ expiresAt: new Date(NOW + ANONYMOUS_DRAFT_TTL_MS + 1).toISOString() })], ['future timestamp', rawRecord({ updatedAt: new Date(NOW + 120_000).toISOString(), expiresAt: new Date(NOW + 120_000 + ANONYMOUS_DRAFT_TTL_MS).toISOString() })], ['missing resume step', rawRecord({}, { resumeStep: undefined })], ['category mismatch', rawRecord({}, { issueType: 'collision' })], ['over-limit text', rawRecord({}, { summary: 'x'.repeat(1001) })], ['medical inflection', rawRecord({}, { summary: 'I fractured my arm.' })], ['concussion', rawRecord({}, { summary: 'Concussion after the collision.' })], ['broken bone', rawRecord({}, { summary: 'I broke my arm.' })], ['bleeding inflection', rawRecord({}, { summary: 'I was bleeding after the crash.' })], ['medical counterparty', rawRecord({}, { counterparty: 'Dr Smith treated my fracture.' })], ['Serbian hospital inflection', rawRecord({}, { counterparty: 'Gradskoj bolnici' })], ['Serbian injury inflection', rawRecord({}, { summary: 'Povređen vozač.' })], ['Serbian Cyrillic injury', rawRecord({}, { summary: 'Повређен возач.' })], ['Cyrillic treatment', rawRecord({}, { summary: 'Лечење и прелом.' })], ['Cyrillic pain morphology', rawRecord({}, { summary: 'Имам бола, болу и болови.' })], ['Macedonian pain inflection', rawRecord({}, { summary: 'Болката продолжува.' })]];
// prettier-ignore
describe('anonymous Free Start recovery', () => {
  ...
  // prettier-ignore
  it('round-trips only eligible allowlisted facts', () => { expect(writeAnonymousDraft(localStorage, snapshot, null, NOW).status).toBe('saved'); expect(readAnonymousDraft(localStorage, NOW + 1)).toEqual(expect.objectContaining({ status: 'available', record: expect.objectContaining(snapshot) })); const raw = localStorage.getItem(ANONYMOUS_DRAFT_KEY)!; ['email', 'tenant', 'member', 'claimPack', 'activeId'].forEach(value => expect(raw).not.toContain(value)); expect(localStorage).toHaveLength(1); expect(createAnonymousDraftSnapshot('injury', snapshot.draft, 'details')).toBeNull(); for (const text of ['Hospitality provider', 'Lënda lidhet me dëmin e pronës', 'Bolji servis', 'Боља понуда', 'Broken windshield', 'Broken pipe', 'Paint damage', 'Insurance treaty']) expect(createAnonymousDraftSnapshot('property', { ...snapshot.draft, summary: text }, 'details')).not.toBeNull(); localStorage.clear(); for (const draft of [{ ...snapshot.draft, summary: 'Hospital treatment' }, { ...snapshot.draft, summary: 'I fractured my arm.' }, { ...snapshot.draft, summary: 'I broke my arm.' }, { ...snapshot.draft, summary: 'Chemotherapy for cancer.' }, { ...snapshot.draft, summary: 'Contact owner@example.com.' }, { ...snapshot.draft, summary: 'OTP 123456' }, { ...snapshot.draft, summary: 'Claim number: CLM-12345' }, { ...snapshot.draft, summary: '<script>alert(1)</script>' }, { ...snapshot.draft, counterparty: 'Dr Smith treated my fracture.' }, { ...snapshot.draft, counterparty: 'Gradskoj bolnici' }, { ...snapshot.draft, summary: 'Povređen vozač.' }, { ...snapshot.draft, summary: 'Повређен возач.' }, { ...snapshot.draft, summary: 'Лечење и прелом.' }, { ...snapshot.draft, summary: 'Imam bola, bolu i bolovi.' }, { ...snapshot.draft, summary: 'Имам бола, болу и болови.' }, { ...snapshot.draft, summary: 'Болки по несреќата.' }]) expect(writeAnonymousDraft(localStorage, { ...snapshot, draft }, null, NOW).status).toBe('unavailable'); expect(localStorage).toHaveLength(0); });
  ...
  // prettier-ignore
  it.each(['storage', 'locks'] as const)('requires an explicit fresh epoch when the original sibling discard hits %s denial', async failure => { const onReset = vi.fn(), props = { activeId: null, category: 'property' as const, draft: snapshot.draft, lifecycleState: 'idle' as const, neutralHost: globalThis.location.host, onReset, onRestore: vi.fn(), resetCategory: 'property' as const, step: 'details' as const }, hook = renderHook(value => useAnonymousDraftRecovery(value), { initialProps: props }); await waitFor(() => expect(hook.result.current.state).toBe('saved')); const available = localStorage; localStorage.removeItem(ANONYMOUS_DRAFT_KEY); if (failure === 'storage') vi.spyOn(globalThis, 'localStorage', 'get').mockImplementation(() => { throw new DOMException('blocked'); }); else installLocks(vi.fn().mockRejectedValue(new Error('denied'))); act(() => globalThis.dispatchEvent(storageEvent(ANONYMOUS_DRAFT_KEY, available, null))); await waitFor(() => expect(hook.result.current.state).toBe('discarded')); expect(hook.result.current.enabled).toBe(false); vi.restoreAllMocks(); installLocks(); hook.rerender({ ...props, draft: { ...snapshot.draft, summary: 'Same epoch stays off.' } }); await act(async () => new Promise(resolve => setTimeout(resolve, 20))); expect(localStorage.getItem(ANONYMOUS_DRAFT_KEY)).toBeNull(); act(() => hook.result.current.discard()); await waitFor(() => expect(onReset).toHaveBeenCalledOnce()); expect(hook.result.current.enabled).toBe(true); hook.rerender({ ...props, draft: { ...snapshot.draft, summary: 'Fresh epoch.' } }); await waitFor(() => expect(localStorage.getItem(ANONYMOUS_DRAFT_KEY)).toContain('Fresh epoch.')); });

  // prettier-ignore
  it('re-offers an unaccepted copy after temporary denial instead of overwriting it', async () => { writeAnonymousDraft(localStorage, snapshot, null, NOW); const props: HookProps = { activeId: null, category: null, draft: snapshot.draft, lifecycleState: 'idle', neutralHost: globalThis.location.host, onReset: vi.fn(), onRestore: vi.fn(), resetCategory: null, step: 'details' }, hook = renderHook(value => useAnonymousDraftRecovery(value), { initialProps: props }); await waitFor(() => expect(hook.result.current.state).toBe('offer')); const available = localStorage, before = available.getItem(ANONYMOUS_DRAFT_KEY); vi.spyOn(globalThis, 'localStorage', 'get').mockImplementation(() => { throw new DOMException('blocked'); }); act(() => globalThis.dispatchEvent(storageEvent(ANONYMOUS_DRAFT_KEY, available))); await waitFor(() => expect(hook.result.current.state).toBe('retained')); expect(hook.result.current.offer).toBeNull(); vi.restoreAllMocks(); hook.rerender({ ...props, category: 'property', draft: { ...snapshot.draft, summary: 'Must not overwrite offer A.' } }); await waitFor(() => expect(hook.result.current.state).toBe('offer')); expect(hook.result.current.offer?.draft.summary).toBe(snapshot.draft.summary); expect(localStorage.getItem(ANONYMOUS_DRAFT_KEY)).toBe(before); });
  ...
  it('never promotes a newer edit into browser storage after explicit opt-out', async () => {
    ...
    const hook = renderHook(props => useAnonymousDraftRecovery(props), { initialProps: initial });
    await waitFor(() => expect(hook.result.current.ready).toBe(true));

    writeAnonymousDraft(localStorage, snapshot, null, NOW);
    act(() => globalThis.dispatchEvent(storageEvent(ANONYMOUS_DRAFT_KEY, localStorage)));
    await waitFor(() => expect(hook.result.current.state).toBe('saved'));
    expect(hook.result.current.enabled).toBe(false);

    hook.rerender({ ...initial, lifecycleState: 'saving' });
    const edited = {
      ...initial,
      draft: { ...snapshot.draft, summary: 'Must remain off this device.' },
      lifecycleState: 'saving' as const,
    };
    hook.rerender(edited);
    hook.rerender({
      ...edited,
      activeFingerprint: draftFingerprint(initial.category, initial.draft, initial.step),
      activeId: 'server-draft',
      lifecycleState: 'saved',
    });

    await waitFor(() => expect(hook.result.current.state).toBe('secure'));
    expect(localStorage.getItem(ANONYMOUS_DRAFT_KEY)).toBeNull();
  });
  ...
  // prettier-ignore
  it('does not cancel a pending write and persists the edit that completes reconciliation', async () => { let grant: (() => void) | undefined; const request = vi.fn((_name, options, callback) => request.mock.calls.length === 2 ? new Promise((resolve, reject) => { grant = () => resolve(callback()); options.signal.addEventListener('abort', () => reject(new DOMException('aborted', 'AbortError'))); }) : Promise.resolve(callback())); installLocks(request); const props: HookProps = { activeId: null, category: null, draft: snapshot.draft, lifecycleState: 'idle', neutralHost: globalThis.location.host, onReset: vi.fn(), onRestore: vi.fn(), resetCategory: null, step: 'details' }; const hook = renderHook(value => useAnonymousDraftRecovery(value), { initialProps: props }); await waitFor(() => expect(request).toHaveBeenCalledOnce()); hook.rerender({ ...props, category: 'property' }); await waitFor(() => expect(request).toHaveBeenCalledTimes(2)); act(() => globalThis.dispatchEvent(storageEvent(ANONYMOUS_DRAFT_KEY))); await act(async () => Promise.resolve()); expect(request).toHaveBeenCalledTimes(2); act(() => grant?.()); await waitFor(() => expect(localStorage.getItem(ANONYMOUS_DRAFT_KEY)).toContain('Water damaged two rooms.')); await waitFor(() => expect(hook.result.current.state).toBe('saved')); vi.spyOn(globalThis, 'localStorage', 'get').mockImplementation(() => { throw new DOMException('blocked'); }); act(() => globalThis.dispatchEvent(storageEvent(ANONYMOUS_DRAFT_KEY))); await waitFor(() => expect(hook.result.current.state).toBe('retained')); vi.restoreAllMocks(); installLocks(); hook.rerender({ ...props, category: 'property', draft: { ...snapshot.draft, summary: 'Reconciled edit persists.' } }); await waitFor(() => expect(localStorage.getItem(ANONYMOUS_DRAFT_KEY)).toContain('Reconciled edit persists.')); });
  // prettier-ignore
