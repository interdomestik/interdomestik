...
describe('anonymous Free Start recovery', () => {
  ...
  // prettier-ignore
  it('does not let a sibling storage event cancel the newer pending local write', async () => { let grant: (() => void) | undefined; const request = vi.fn((_name, options, callback) => request.mock.calls.length === 2 ? new Promise((resolve, reject) => { grant = () => resolve(callback()); options.signal.addEventListener('abort', () => reject(new DOMException('aborted', 'AbortError'))); }) : Promise.resolve(callback())); installLocks(request); const props: HookProps = { activeId: null, category: null, draft: snapshot.draft, lifecycleState: 'idle', neutralHost: globalThis.location.host, onReset: vi.fn(), onRestore: vi.fn(), resetCategory: null, step: 'details' }; const hook = renderHook(value => useAnonymousDraftRecovery(value), { initialProps: props }); await waitFor(() => expect(request).toHaveBeenCalledOnce()); hook.rerender({ ...props, category: 'property' }); await waitFor(() => expect(request).toHaveBeenCalledTimes(2)); act(() => globalThis.dispatchEvent(new StorageEvent('storage', { key: ANONYMOUS_DRAFT_KEY }))); await waitFor(() => expect(request).toHaveBeenCalledTimes(3)); act(() => grant?.()); await waitFor(() => expect(localStorage.getItem(ANONYMOUS_DRAFT_KEY)).toContain('Water damaged two rooms.')); expect(hook.result.current.state).toBe('saved'); });
});
