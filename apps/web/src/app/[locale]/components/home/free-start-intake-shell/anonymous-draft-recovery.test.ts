...
// prettier-ignore
function storageEvent(key: string | null, storageArea?: Storage) { const event = new Event('storage') as StorageEvent; Object.defineProperties(event, { key: { value: key }, storageArea: { value: storageArea } }); return event; }
// prettier-ignore
...
// prettier-ignore
const invalidRecords = [['malformed', '{'], ['unknown version', rawRecord({ version: 2 })], ['expired', rawRecord({ expiresAt: new Date(NOW - 1).toISOString() })], ['extended TTL', rawRecord({ expiresAt: new Date(NOW + ANONYMOUS_DRAFT_TTL_MS + 1).toISOString() })], ['future timestamp', rawRecord({ updatedAt: new Date(NOW + 120_000).toISOString(), expiresAt: new Date(NOW + 120_000 + ANONYMOUS_DRAFT_TTL_MS).toISOString() })], ['missing resume step', rawRecord({}, { resumeStep: undefined })], ['category mismatch', rawRecord({}, { issueType: 'collision' })], ['over-limit text', rawRecord({}, { summary: 'x'.repeat(1001) })], ['medical inflection', rawRecord({}, { summary: 'I fractured my arm.' })], ['bleeding inflection', rawRecord({}, { summary: 'I was bleeding after the crash.' })], ['medical counterparty', rawRecord({}, { counterparty: 'Dr Smith treated my fracture.' })], ['Serbian hospital inflection', rawRecord({}, { counterparty: 'Gradskoj bolnici' })]];
...
describe('anonymous Free Start recovery', () => {
  ...
  // prettier-ignore
  it('requires an explicit fresh epoch after a sibling discard', async () => { const onReset = vi.fn(), props = { activeId: null, category: 'property' as const, draft: snapshot.draft, lifecycleState: 'idle' as const, neutralHost: globalThis.location.host, onReset, onRestore: vi.fn(), resetCategory: 'property' as const, step: 'details' as const }; const hook = renderHook(value => useAnonymousDraftRecovery(value), { initialProps: props }); await waitFor(() => expect(hook.result.current.state).toBe('saved')); act(() => { localStorage.removeItem(ANONYMOUS_DRAFT_KEY); globalThis.dispatchEvent(storageEvent(ANONYMOUS_DRAFT_KEY)); }); await waitFor(() => expect(hook.result.current.state).toBe('discarded')); expect(hook.result.current.enabled).toBe(false); act(() => hook.result.current.discard()); await waitFor(() => expect(onReset).toHaveBeenCalledOnce()); expect(hook.result.current.enabled).toBe(true); hook.rerender({ ...props, draft: { ...snapshot.draft, summary: 'Fresh epoch.' } }); await waitFor(() => expect(localStorage.getItem(ANONYMOUS_DRAFT_KEY)).toContain('Fresh epoch.')); });

  // prettier-ignore
  it('withdraws an offer but reports its known copy as retained when storage becomes unavailable', async () => { writeAnonymousDraft(localStorage, snapshot, null, NOW); const props = { activeId: null, category: null, draft: snapshot.draft, lifecycleState: 'idle' as const, neutralHost: globalThis.location.host, onReset: vi.fn(), onRestore: vi.fn(), resetCategory: null, step: 'details' as const }; const hook = renderHook(value => useAnonymousDraftRecovery(value), { initialProps: props }); await waitFor(() => expect(hook.result.current.state).toBe('offer')); const available = localStorage; vi.spyOn(globalThis, 'localStorage', 'get').mockImplementation(() => { throw new DOMException('blocked'); }); act(() => globalThis.dispatchEvent(storageEvent(ANONYMOUS_DRAFT_KEY, available))); await waitFor(() => expect(hook.result.current.state).toBe('retained')); expect(hook.result.current.offer).toBeNull(); expect(hook.result.current.enabled).toBe(false); });
