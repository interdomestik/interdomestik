...
describe('anonymous Free Start recovery', () => {
  ...
  // prettier-ignore
  it('preserves a same-millisecond user edit when a queued reset removes the prior revision first', async () => { vi.spyOn(Date, 'now').mockReturnValue(NOW); let grant: (() => void) | undefined; const request = vi.fn((_name, options, callback) => request.mock.calls.length === 3 ? new Promise((resolve, reject) => { grant = () => resolve(callback()); options.signal.addEventListener('abort', () => reject(new DOMException('aborted', 'AbortError'))); }) : Promise.resolve(callback())); installLocks(request); const props: HookProps = { activeId: null, category: 'property', draft: snapshot.draft, lifecycleState: 'idle', neutralHost: globalThis.location.host, onReset: vi.fn(), onRestore: vi.fn(), resetCategory: 'property', step: 'preview' }; const hook = renderHook(value => useAnonymousDraftRecovery(value), { initialProps: props }); await waitFor(() => expect(hook.result.current.state).toBe('saved')); hook.rerender({ ...props, draft: { ...snapshot.draft, summary: 'Same-millisecond edit.' } }); await waitFor(() => expect(request).toHaveBeenCalledTimes(3)); localStorage.removeItem(ANONYMOUS_DRAFT_KEY); act(() => grant?.()); await waitFor(() => expect(localStorage.getItem(ANONYMOUS_DRAFT_KEY)).toContain('Same-millisecond edit.')); });
  // prettier-ignore
