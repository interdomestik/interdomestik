...
export type PublicJourneyLiveMode = 'off' | 'polite' | 'assertive';

type PublicJourneyFrameProps = Readonly<{
  children: ReactNode;
  contentRef: RefObject<HTMLDivElement | null>;
  eyebrow: string;
  headingId: string;
  intro: string;
  liveMode: PublicJourneyLiveMode;
  privacy: string;
  testId: string;
}>;

export function PublicJourneyFrame(props: PublicJourneyFrameProps) {
  return (
    <section
    ...
    >
      <div className="mx-auto grid max-w-6xl gap-8 px-4 py-10 sm:px-6 md:py-14 lg:grid-cols-[0.72fr_1.28fr] lg:gap-14">
        <div className="space-y-5">
          ...
          <p className="max-w-md text-base leading-7 text-[#334D5C]">{props.intro}</p>
          <div className="flex gap-3 border-t border-[#B8C7C7] pt-5 text-[#334D5C]">
            ...
            <p className="text-base leading-7">{props.privacy}</p>
          </div>
        </div>
        <div
          ref={props.contentRef}
          aria-live={props.liveMode}
          aria-atomic="true"
