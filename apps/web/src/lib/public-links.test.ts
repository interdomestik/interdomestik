...
describe('public-links', () => {
  it('replaces localhost fallback origins with the current live origin', () => {
    expect(
      normalizePublicLink('https://localhost:3000?ref=ABC123', 'https://mk.127.0.0.1.nip.io:3000')
    ).toBe('https://mk.127.0.0.1.nip.io:3000/?ref=ABC123');
  });
  ...
  it('keeps the existing path and query when normalizing share URLs', () => {
    expect(
      normalizeWhatsAppShareUrl(
        ...
        'https://ks.127.0.0.1.nip.io:3000'
      )
    ).toBe(
      'https://wa.me/?text=Join+Asistenca+with+my+referral+link+https%3A%2F%2Fks.127.0.0.1.nip.io%3A3000%2F%3Fref%3DABC123'
    );
  });
});
