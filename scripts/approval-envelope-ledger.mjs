...
const CHILD_LIST =
  'B0-authority-bootstrap,B1-cd-guard,S1A-skill-authority,S1B-routing-standard,S1C-typed-modularity-guard,S2-mcp-identity,S3-exact-authority,S4A-terminal-delivery,S4B-reviewer-policy';
export const CHILDREN = CHILD_LIST.split(',');
...
const STATE_FIELDS =
  'schemaVersion,programId,revision,status,childId,runtimeAuthorized,successorsBlocked,envelopeSha256,approvalReceiptSha256,boundary,evidenceRef,previousOperationSha256';
const STATE_KEYS = STATE_FIELDS.split(',');
const EVIDENCE_FIELDS =
  'schemaVersion,programId,revision,event,fromChild,toChild,boundary,previousOperationSha256,proofSha256';
const EVIDENCE_KEYS = EVIDENCE_FIELDS.split(',');
...
function validate(state, initial = false) {
  ...
  const previous = state.previousOperationSha256;
  store.must(first ? previous === null : store.hex(previous, 64));
  const closing = ['closed', 'closeout_required'].includes(state.status);
  store.must(!closing || state.childId === CHILDREN.at(-1));
...
function commit(root, expected, state, options = {}) {
  ...
  const current =
    expected === null && !options.recover ? null : read(root, Boolean(options.recover));
  ...
  try {
    ...
    for (const [path, content, directory] of artifacts) {
      const exists = store.exists(path);
      mutated = true;
      if (!exists) store.writeNew(path, content);
      else store.must(store.regular(path) && store.text(path) === content, 'partial mismatch');
      store.flush(directory);
    }
    const temporary = `${paths.target}.tmp-${operation}`;
    mutated ||= store.exists(temporary);
    if (!store.exists(temporary)) store.writeNew(temporary, store.body(record));
    else store.must(store.regular(temporary) && store.text(temporary) === store.body(record));
    store.must(store.same(observe(), current), 'stale authority state');
...
export function resolveLedger(root = AUTHORITY_ROOT, live) {
  try {
