...
test('e2e gate scripts keep full and fast lanes distinct', () => {
  ...
  assert.equal(
    fastCheck,
    'pnpm --filter @interdomestik/web run build:ci && node scripts/run-with-default-db-url.mjs pnpm e2e:state:setup && node scripts/run-with-default-db-url.mjs pnpm e2e:gate:pr:fast'
  );
