    ...
    for (const [expected, cases] of [
      [
        ...
        [
          ...
          `const {key='NAME'}={};x[key]`,
          `const {key:alias='NAME'}={};x[alias]`,
          `const [key='NAME']=[];x[key]`,
          `function f(key='NAME'){x[key]}`,
          `const key='NAME';function f(alias=key){const key='safe';x[alias]}`,
          `function f({key='NAME'}={}){x[key]}`,
          `const alias=key;const {key=terminal}={};const terminal='NAME';x[alias]`,
        ],
      ],
      [
        false,
        [
          ...
          `const a=b,b=a;x[a]`,
          `<Widget NAME='NAME'/>`,
          `const {label='NAME'}={};analytics.track(label)`,
          `function f(label='NAME'){return label}`,
          `const {key='NAME'}={};function f(key='safe'){x[key]}`,
          `function f(key='NAME'){const inner=()=>{const key='safe';x[key]}}`,
          `const {a=b,b=a}={};x[a]`,
          `function f(key){x[key]}`,
          `const key='safe';function f(alias=key){const key='NAME';x[alias]}`,
        ],
      ],
    ])
