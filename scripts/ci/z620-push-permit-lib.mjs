...
function signature(payload, key) {
  return createHmac('sha256', key).update(JSON.stringify(stable(payload))).digest('hex');
}
...
export function verifyPermit(permit, { key, currentSha, now = Date.now() } = {}) {
  if (!key || !permit?.signature) return false;
  if (permit.sha !== currentSha || Date.parse(permit.expiresAt) <= now) return false;
