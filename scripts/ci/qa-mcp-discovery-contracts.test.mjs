...
test('repo QA MCP repo helpers return structured results for faster agent inspection', async () => {
  ...
  try {
    ...
    const projectMapResult = await client.callTool('project_map', {
      maxDepth: 1,
      repoRoot: rootDir,
    });
    assert.equal(projectMapResult.structuredContent.tool, 'project_map');

    const branchResult = await client.callTool('git_branch_info', { repoRoot: rootDir });
    assert.equal(branchResult.structuredContent.tool, 'git_branch_info');
    assert.ok(branchResult.structuredContent.head);

    const statusResult = await client.callTool('git_status_compact', { repoRoot: rootDir });
    ...
    const changedResult = await client.callTool('changed_files', { repoRoot: rootDir });
