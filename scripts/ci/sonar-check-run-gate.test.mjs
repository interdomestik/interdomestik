...
function createSonarApiHandler(responseByPath) {
  return (request, response) => {
    const payload = responseByPath[request.url];
    if (payload) {
...
function startSonarApiServer(responseByPath) {
  return new Promise(resolve => {
    const server = http.createServer(createSonarApiHandler(responseByPath));
...
const pullRequestsPath = '/api/project_pull_requests/list?project=interdomestik_interdomestik';
...
`,
  ...
  const server = await startSonarApiServer({
    [pullRequestsPath]: {
      pullRequests: [
        {
          key: '307',
...
test('sonar check-run gate ignores stale PR quality gate data until Sonar analyzes the current sha', async () => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'sonar-check-run-gate-'));
  const binDir = path.join(tempRoot, 'bin');
  const responsesDir = path.join(tempRoot, 'responses');
  const evidenceDir = path.join(tempRoot, 'evidence');
...
`,
  ...
  try {
    ...
    assert.match(result.stdout, /Sonar PR analysis still points at stale-sonar-sha; waiting for test-sha/);

    const summaryPath = path.join(evidenceDir, 'notes', 'sonar-summary.md');
    const summary = fs.readFileSync(summaryPath, 'utf8');
    assert.match(summary, /source: GitHub check suite/);
    assert.doesNotMatch(summary, /source: Sonar quality gate API/);
  } finally {
    await server.close();
  }
});
