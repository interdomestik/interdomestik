...
} = require('./release-gate/pilot-artifacts.ts');
...
function git(rootDir, args) {
  return execGit(args, { cwd: rootDir, encoding: 'utf8' }).trim();
}
...
function assertPathExistsInHead(rootDir, repoPath, label) {
  ...
  try {
    execGit(['cat-file', '-e', `HEAD:${repoPath}`], {
      cwd: rootDir,
      stdio: 'ignore',
    });
  } catch {
...
function readExistingTag(rootDir, tagName) {
  try {
    execGit(['show-ref', '--verify', '--quiet', `refs/tags/${tagName}`], {
      cwd: rootDir,
      stdio: 'ignore',
    });
    return {
...
function createPilotReadyTag(args) {
  ...
  execGit(['tag', '-a', entry.tagName, entry.commitSha, '-m', buildTagMessage(entry)], {
    cwd: entry.rootDir,
    stdio: 'ignore',
  });
