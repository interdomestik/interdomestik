...
function initialize(authority) {
  ...
  state.evidenceRef = evidenceReference(evidence);
  return installLedger(authority, null, state, { evidence });
}
...
function transitionFixture(label) {
  ...
  const target = proposal(current, 'merged_consumed', 'merge_consumed');
  return { authority, current, target };
}
test('executes the exact declared topology and terminal failure edge', () => {
  ...
  const closedLive = { ...live, disposition: 'closed' };
  assert.equal(resolveLedger(authority, closedLive).reason, 'invalid_state');
  ...
  current = advance(authority, current, 'closed', 'success_closeout');
...
test('recovery is exact, idempotent, and rejects unsafe filesystem state', () => {
  ...
  const installed = initialize(initial);
  const { operationSha256, ...state } = installed;
  const evidence = JSON.parse(store.text(join(initial, state.evidenceRef)));
  lock(initial, null, { state }, token);
  const repeated = recover(initial, null, { state, evidence }, token);
  assert.equal(repeated.operationSha256, operationSha256);
