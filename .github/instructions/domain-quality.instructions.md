...
- Reuse shared auth and tenant helpers such as `ensureTenantId`, `withTenant(...)`, and explicit session guards already used in the repo (for example, the tenant admin helpers in `packages/domain-users`) instead of duplicating scope logic.
